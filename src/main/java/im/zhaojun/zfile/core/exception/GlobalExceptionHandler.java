...
public class GlobalExceptionHandler {
  ...
  @Resource private FrontIndexController frontIndexController;
  ...
  /**
   * 所有未找到的页面都跳转到首页, 用户解决 vue history 直接访问 404 的问题 同时, 读取 index.html 文件, 修改 title 和 favicon 后返回.
   *
  ...
  @ResponseBody
  @ResponseStatus
  public AjaxJson<String> initializeStorageSourceBizException(
      InitializeStorageSourceBizException e) {
  ...
  @ResponseBody
  @ResponseStatus
  public AjaxJson<String> storageSourceFileForbiddenAccessBizException(
      StorageSourceFileForbiddenAccessBizException e) {
  ...
  @ResponseBody
  @ResponseStatus
  public AjaxJson<String> storageSourceIllegalOperationBizException(
      StorageSourceIllegalOperationBizException e) {
  ...
  public AjaxJson<String> uploadFileFailSystemException(UploadFileFailSystemException e) {
    log.warn(
        "上传文件失败, 存储类型: {}, 上传路径: {}, 输入流可用字节数: {}, 响应码: {}, 响应体: {}",
        e.getStorageTypeEnum(),
        e.getUploadPath(),
        e.getInputStreamAvailable(),
        e.getResponseCode(),
        e.getResponseBody());
    return new AjaxJson<>(e.getCode(), e.getMessage());
  }
  ...
  public AjaxJson<Map<String, String>> handleValidException(Exception e) {
    ...
    return new AjaxJson<>(
        ErrorCode.BIZ_BAD_REQUEST.getCode(), ErrorCode.BIZ_BAD_REQUEST.getMessage(), errorMap);
  }
