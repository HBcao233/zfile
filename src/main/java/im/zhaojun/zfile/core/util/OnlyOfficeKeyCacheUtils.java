...
public class OnlyOfficeKeyCacheUtils {
  ...
  private static final Cache<OnlyOfficeFile, String> ONLY_OFFICE_FILE_KEY_MAP =
      CacheUtil.newLRUCache(10000);

  /** 存储 OnlyOffice Key 与文件信息的映射关系. 最多存储 10000 个 Key, 防止内存溢出. */
  private static final Cache<String, OnlyOfficeFile> ONLY_OFFICE_KEY_FILE_MAP =
      CacheUtil.newLRUCache(10000);
   ...
   *
   * @param onlyOfficeFile OnlyOffice 文件信息
   * @return 该文件唯一标识
   */
  ...
  public static List<OnlyOfficeFile> removeByFolder(OnlyOfficeFile onlyOfficeFile) {
    ...
    Iterator<CacheObj<OnlyOfficeFile, String>> cacheObjIterator =
        ONLY_OFFICE_FILE_KEY_MAP.cacheObjIterator();
    while (cacheObjIterator.hasNext()) {
      ...
      if (cacheOnlyOfficeFile.getStorageKey().equals(onlyOfficeFile.getStorageKey())
          && StringUtils.startWith(
              cacheOnlyOfficeFile.getPathAndName(), onlyOfficeFile.getPathAndName())) {
