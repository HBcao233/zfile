...
public class PatternMatcherUtils {
  ...
  /**
   * 兼容模式的 glob 表达式匹配. 默认的 glob 表达式是不支持以下情况的:<br>
   *
   * <ul>
   *   <li>pattern: /a/**
   *   <li>test1: /a
   *   <li>test2: /a/
   *       <ul>
   *         <p>test1 和 test 2 均无法匹配这种情况, 此方法兼容了这种情况, 即对 test 内容后拼接 "/xx"(其实任意字符都可以), 使其可以匹配上
   *         pattern.
   *         <p><strong>注意：</strong>但此方法对包含文件名的情况无效, 仅支持 test 为 路径的情况.
   *
   * @param pattern glob 规则表达式
   * @param test 匹配内容
   * @return 是否匹配.
   */
