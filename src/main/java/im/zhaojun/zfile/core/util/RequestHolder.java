...
public class RequestHolder {
   ...
   * @param fileName 文件名称
   * @param fileSize 文件大小
   * @param isPartialContentFromInputStream 表示输入流是否为部分内容。 当该变量为 true 时，表示输入流已经根据 range 规则从存储源获取部分内容。
  ...
  public static void writeFile(
      ...
      String fileName,
      Long fileSize,
      boolean isPartialContentFromInputStream,
    ...
    try (InputStream innerInputStream = inputStream) {
      ...
      if (forceDownload) {
        ...
        response.addHeader(
            ...
            "attachment;filename=" + StringUtils.encodeAllIgnoreSlashes(fileName));
      } else {
        response.addHeader(
            HttpHeaders.CONTENT_DISPOSITION,
            "inline;filename=" + StringUtils.encodeAllIgnoreSlashes(fileName));
      ...
      if (fileSize != null && fileSize > 0) {
        ...
        if (response.getStatus() == HttpServletResponse.SC_PARTIAL_CONTENT) {
          response.setHeader(
              ...
              "bytes " + startPos + "-" + endPos + StringUtils.SLASH + fileSize);
        }
  ...
  public static boolean isAxiosRequest() {
    ...
    String axiosRequest =
        JakartaServletUtil.getHeaderIgnoreCase(request, ZFileHttpHeaderConstant.AXIOS_REQUEST);
  ...
  public static String getRequestServerAddress() {
    ...
    String serverName;
    String hostHeader =
        StringUtils.isNotBlank(forwardedHost) ? forwardedHost : request.getHeader("Host");
