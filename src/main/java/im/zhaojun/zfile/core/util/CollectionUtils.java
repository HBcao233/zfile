...
public class CollectionUtils {
   ...
   */
  @Nullable public static <T> T getLast(@Nullable List<T> list) {
    if (isEmpty(list)) {
      return null;
    }
   ...
   * @param values 被转换的 Iterable
   * @param map 转换后的 Value 存放的 Map, 如果为 {@code null} 则新建一个 HashMap
   * @param keyFunc 生成 Map 的 Key 的函数
   * @return 转换后的 Map
   */
  public static <K, V> Map<K, V> toMap(
      final @Nullable Iterable<V> values,
      final @Nullable Map<K, V> map,
      final @Nullable Func1<V, K> keyFunc) {
