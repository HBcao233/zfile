...
public class SsoService {
  ...
  public SsoConfig saveOrUpdate(SsoConfig ssoConfig) {
    boolean providerIsDuplicate =
        checkDuplicateProvider(ssoConfig.getId(), ssoConfig.getProvider());
        ...
        new HashMap<>() {
          {
            ...
            put(
                "redirect_uri",
                systemConfigService.getAxiosFromDomainOrSetting()
                    + StrUtil.format(REDIRECT_URI, provider));
            put("state", state);
  ...
  public String callbackHandler(String provider, String code) {
    ...
    // 获取 Access Token
    Map<String, String> tokenParamsMap =
        new HashMap<>() {
          {
            put("code", code);
            put("client_id", config.getClientId());
    ...
    HttpResponse getTokenResponse =
        HttpUtil.createPost(config.getTokenUrl())
    ...
    TokenResponse token =
        JSON.parseObject(tokenStr, TokenResponse.class, JSONReader.Feature.SupportSmartMatch);
    ...
    HttpResponse userInfoResponse =
        HttpUtil.createGet(config.getUserInfoUrl()).bearerAuth(token.getAccessToken()).execute();
    String userInfoStr = userInfoResponse.body();
    if (log.isDebugEnabled()) {
      log.debug(
          "[UserInfo] 单点登录服务商处请求 {} 的用户信息: {}，将尝试通过 {} 表达式获取字段",
          config.getUserInfoUrl(),
          userInfoStr,
          config.getBindingField());
    }
    if (!userInfoResponse.isOk()) {
      log.error(
          "单点登录服务商 {} 返回错误: {}, 错误信息: {}", provider, userInfoResponse.getStatus(), userInfoStr);
      throw new ErrorPageBizException(
          "从单点登录服务商获取用户信息失败: " + userInfoResponse.getStatus() + ", " + userInfoStr);
    }

    Object bindingField =
        JSON.parseObject(userInfoStr, JSONReader.Feature.SupportSmartMatch)
            .getByPath(config.getBindingField());
    if (log.isDebugEnabled()) {
