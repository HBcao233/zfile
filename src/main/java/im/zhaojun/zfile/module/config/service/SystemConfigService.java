...
public class SystemConfigService {
  ...
  @Resource private SystemConfigModifyHandlerChain systemConfigModifyHandlerChain;
   ...
   * 更新系统设置, 并清空缓存中的内容.
   *
   * @param systemConfigDTO 系统设置 dto
   */
  ...
  public synchronized void updateSystemConfig(SystemConfigDTO systemConfigDTO) {
    ...
    Map<String, SystemConfig> systemConfigMapInDb =
        CollectionUtils.toMap(systemConfigListInDb, null, SystemConfig::getName);
    ...
    updateSystemConfigList.forEach(
        systemConfigInForm -> {
  ...
  public synchronized String getAesHexKeyOrGenerate() {
    SystemConfigDTO systemConfigDTO =
        ((SystemConfigService) AopContext.currentProxy()).getSystemConfig();
    ...
    if (StringUtils.isEmpty(aesHexKey)) {
      ...
      SystemConfig loginVerifyModeConfig =
          systemConfigMapper.findByName(SystemConfigConstant.AES_HEX_KEY);
  ...
  public String getRealFrontDomain() {
    SystemConfigDTO systemConfigDTO =
        ((SystemConfigService) AopContext.currentProxy()).getSystemConfig();
    return StringUtils.firstNonNull(
        systemConfigDTO.getFrontDomain(),
        getAxiosFromDomainOrSetting(),
        RequestHolder.getOriginAddress());
  }

  /**
   * 优先级： 1. 如果设置了强制后端地址，则使用强制后端地址。 2. 如果请求中有 axios-from 参数，则使用该参数。 3. 如果没有强制后端地址和 axios-from
   * 参数，则使用请求的服务器地址（如果经过多个代理，可能不是实际的后端地址）。
   *
   * @return 后端站点地址
   */
