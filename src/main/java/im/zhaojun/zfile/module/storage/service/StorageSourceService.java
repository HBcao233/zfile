...
public class StorageSourceService {
  ...
  @Resource private StorageSourceMapper storageSourceMapper;

  @Resource private StorageSourceConvert storageSourceConvert;

  @Resource private StorageSourceConfigService storageSourceConfigService;

  @Resource private PasswordConfigService passwordConfigService;

  @Resource private UserStorageSourceService userStorageSourceService;

  @Resource private ReadmeConfigService readmeConfigService;

  @Resource private ApplicationEventPublisher applicationEventPublisher;

  @Resource private ZFileCacheManager zfileCacheManager;
  ...
  public List<StorageSource> findAllEnableOrderByOrderNum(Integer userId) {
    ...
    return zfileCacheManager.findAllEnableOrderByOrderNum(
        userId,
        userEnableStorageSourceId ->
            storageSourceMapper.findUserEnableList(userEnableStorageSourceId));
  }
  ...
  public Integer findIdByKey(String storageKey) {
    return Optional.ofNullable(
            ((StorageSourceService) AopContext.currentProxy()).findByStorageKey(storageKey))
        .map(StorageSource::getId)
        .orElse(null);
  }
  ...
  public String findStorageKeyById(Integer id) {
    return Optional.ofNullable(((StorageSourceService) AopContext.currentProxy()).findById(id))
        .map(StorageSource::getKey)
        .orElse(null);
  }
  ...
  public StorageSourceDTO findDTOById(Integer id) {
    ...
    for (StorageSourceConfig storageSourceConfig :
      ...
      if (ReflectUtil.hasField(StorageSourceAllParamDTO.class, storageSourceConfig.getName())) {
        ReflectUtil.setFieldValue(
            storageSourceAllParam, storageSourceConfig.getName(), storageSourceConfig.getValue());
      } else {
        log.warn(
            "数据库中存储源 {} 参数 {} 不存在于存储源参数 DTO 中, 请检查参数名是否正确.", id, storageSourceConfig.getName());
      }
    }
    // 获取数据库对象，转为 dto 对象返回
   ...
   * 交换存储源排序
   *
   * @param updateStorageSortRequestList 更新排序的存储源 id 及排序值列表
   */
  @Transactional(rollbackFor = Exception.class)
   ...
   * 保存存储源基本信息及其对应的参数设置
   *
   * @param saveStorageSourceRequest 存储源 DTO 对象
   */
  @Transactional(rollbackFor = Exception.class)
  public Integer saveStorageSource(SaveStorageSourceRequest saveStorageSourceRequest) {
    ...
    log.info(
        "尝试保存存储源, id: {}, name: {}, key: {}, type: {}",
        saveStorageSourceRequest.getId(),
        saveStorageSourceRequest.getName(),
        saveStorageSourceRequest.getKey(),
        saveStorageSourceRequest.getType().getDescription());

    // 转换为存储源 entity 对象
    StorageSource storageSource =
        storageSourceConvert.saveRequestToEntity(saveStorageSourceRequest);
    storageSource.setSearchMode(SearchModeEnum.SEARCH_ALL_MODE);

    // 如果是更新，则销毁之前的存储源上下文
    if (!isSave) {
      StorageSource dbStorageSource =
          ((StorageSourceService) AopContext.currentProxy())
              .findById(saveStorageSourceRequest.getId());
    ...
    // 保存或更新存储源
    StorageSource dbSaveResult =
        ((StorageSourceService) AopContext.currentProxy()).saveOrUpdate(storageSource);

    log.info(
        "保存存储源成功, id: {}, name: {}, key: {}, type: {}",
        dbSaveResult.getId(),
        dbSaveResult.getName(),
        dbSaveResult.getKey(),
        dbSaveResult.getType().getDescription());
    ...
    List<StorageSourceConfig> storageSourceConfigList =
        storageSourceConfigService.storageSourceAllParamToConfigList(
            storageId, dbSaveResult.getType(), saveStorageSourceRequest.getStorageSourceAllParam());
    storageSourceConfigService.saveBatch(storageId, storageSourceConfigList);
    log.info(
        "保存存储源参数成功，尝试根据参数初始化存储源, id: {}, name: {}, config size: {}",
        dbSaveResult.getId(),
        dbSaveResult.getName(),
        storageSourceConfigList.size());

    // 初始化并检查是否可用
    StorageSourceInitDTO storageSourceInitDTO =
        StorageSourceInitDTO.convert(dbSaveResult, storageSourceConfigList);
    StorageSourceContext.init(storageSourceInitDTO);
    log.info(
        "根据参数初始化存储源成功, id: {}, name: {}, config size: {}",
        dbSaveResult.getId(),
        dbSaveResult.getName(),
        storageSourceConfigList.size());
  ...
  public StorageSource saveOrUpdate(StorageSource storageSource) {
    ...
    if (storageSource.getId() == null) {
      ...
      if (!StringUtils.equals(originStorageSource.getKey(), storageSource.getKey())) {
        ((StorageSourceService) AopContext.currentProxy())
            .clearCacheByStorageKey(originStorageSource.getKey());
      }
      storageSourceMapper.updateById(storageSource);
    }

    // 如果没输入存储源 key, 则自动将 id 设置为 key
    if (StringUtils.isEmpty(storageSource.getKey())
        && !StringUtils.equals(storageSource.getId().toString(), storageSource.getKey())) {
  ...
  public StorageSourceConfigResult getStorageConfigSource(
      FileListConfigRequest fileListConfigRequest) {
    ...
    // 拼接用户目录
    AbstractBaseFileService<IStorageParam> baseFileService =
        StorageSourceContext.getByStorageId(storageId);
    String fullPath =
        StringUtils.concat(
            baseFileService.getCurrentUserBasePath(), fileListConfigRequest.getPath());

    VerifyResultDTO verifyPassword =
        passwordConfigService.verifyPassword(
            storageId, fullPath, fileListConfigRequest.getPassword());

    ReadmeConfig readmeByPath = null;
    if (verifyPassword.isPassed()) {
      // 获取指定存储源路径下的 readme 信息
      readmeByPath =
          readmeConfigService.getByStorageAndPath(
              storageId, fileListConfigRequest.getPath(), storageSource.getCompatibilityReadme());
    } else {
      log.info(
          "文件夹密码验证失败，不获取 readme 信息, storageId: {}, path: {}, password: {}",
          storageId,
          fullPath,
          fileListConfigRequest.getPassword());
    }

    StorageSourceConfigResult storageSourceConfigResult =
        storageSourceConvert.entityToConfigResult(storageSource, readmeByPath);

    // 获取当前用户对该存储源的权限
    HashMap<String, Boolean> permissionMap =
        userStorageSourceService.getCurrentUserPermissionMapByStorageId(storageId);
    ...
    UserStorageSource userStorageSource =
        userStorageSourceService.getByUserIdAndStorageId(
            ZFileAuthUtil.getCurrentUserId(), storageId);
  ...
  public Integer copy(CopyStorageSourceRequest copyStorageSourceRequest) {
    ...
    boolean existByStorageKey =
        ((StorageSourceService) AopContext.currentProxy()).existByStorageKey(toKey);
    ...
    StorageSource storageSource =
        ((StorageSourceService) AopContext.currentProxy()).findById(fromStorageId);
    if (storageSource == null) {
    ...
    StorageSource dbSaveResult =
        ((StorageSourceService) AopContext.currentProxy()).saveOrUpdate(newStorageSource);
    Integer newStorageId = dbSaveResult.getId();
    log.info(
        "复制存储源成功，源 [id: {}, name: {}, key: {}], 复制后 [id: {}, name: {}, key: {}, type: {}]",
        fromStorageId,
        storageSource.getName(),
        storageSource.getKey(),
        newStorageId,
        dbSaveResult.getName(),
        dbSaveResult.getKey(),
        dbSaveResult.getType().getDescription());

    StorageSourceCopyEvent storageSourceCopyEvent =
        new StorageSourceCopyEvent(fromStorageId, newStorageId);
    applicationEventPublisher.publishEvent(storageSourceCopyEvent);

    // 初始化存储源
    List<StorageSourceConfig> storageSourceConfigList =
        storageSourceConfigService.selectStorageConfigByStorageId(newStorageId);
    StorageSourceInitDTO storageSourceInitDTO =
        StorageSourceInitDTO.convert(dbSaveResult, storageSourceConfigList);
    StorageSourceContext.init(storageSourceInitDTO);
    log.info(
        "初始化存储源成功, id: {}, name: {}, config size: {}",
        newStorageId,
        dbSaveResult.getName(),
        storageSourceConfigList.size());
