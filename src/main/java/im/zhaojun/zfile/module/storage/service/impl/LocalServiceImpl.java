...
import im.zhaojun.zfile.module.storage.util.CompressUtils;
...
public class LocalServiceImpl extends AbstractProxyTransferService<LocalParam> {
  ...
  public FileItemResult getFileItem(String pathAndName) {
    ...
    String fullPath =
        StringUtils.concat(param.getFilePath(), getCurrentUserBasePath(), pathAndName);
  ...
  private boolean operateFile(
      String path,
      String name,
      String newPath,
      String newName,
      FileOperatorTypeEnum operatorTypeEnum) {
    ...
    String distPath =
        StringUtils.concat(param.getFilePath(), getCurrentUserBasePath(), newPath, appendName);
  ...
  public boolean compressFile(
      ...
      Long splitSizeMB) {
    checkPathSecurity(path);
    checkNameSecurity(nameList.toArray(new String[0]));
    checkNameSecurity(compressName);

    String dir = StringUtils.concat(param.getFilePath(), getCurrentUserBasePath(), path);
    if (!FileUtil.isDirectory(dir)) {
      throw new BizException(ErrorCode.BIZ_FOLDER_NOT_EXIST);
    }

    String ext = compressType.getExt();
    String filename = findAvailableFilename(dir, compressName, ext);

    return CompressUtils.compress(compressType, dir, nameList, filename, splitSizeMB);
  }
  ...
  public boolean decompressFile(
    ...
    String inputPath = StringUtils.concat(param.getFilePath(), getCurrentUserBasePath(), path);
    String outputPath = StringUtils.concat(param.getFilePath(), getCurrentUserBasePath(), output);
    ...
    String distPath;
    if (createDir) {
      distPath = FileUtil.file(outputPath, FileUtil.mainName(name)).getAbsolutePath();
      if (!FileUtil.isDirectory(distPath)) {
        FileUtil.mkdir(distPath);
      }
    } else {
      distPath = outputPath;
    }

    return CompressUtils.decompress(type, inputPath, name, distPath, password, charset);
  }
  ...
  public ResponseEntity<Resource> downloadToStream(String pathAndName) {
    ...
    if (ObjectUtil.equals(mimeType, MediaType.APPLICATION_OCTET_STREAM)) {
      ...
      headers.put(
          ...
          Collections.singletonList("inline; filename=\"" + filename + "\""));
    }
   ...
   * @param ext 文件扩展名（如 ".txt" 或 "txt"）
   * @param index 当前索引（默认值0）
   * @return 可用的索引值
   */
  public static String findAvailableFilename(String dir, String name, String ext) {
    return findAvailableFilename(dir, name, ext, 0);
  }

  public static String findAvailableFilename(String dir, String name, String ext, int index) {
    // 确保扩展名以点开头
    if (!ext.startsWith(".")) {
      ext = "." + ext;
    }

    String fileName = (index == 0) ? name + ext : String.format("%s (%d)%s", name, index, ext);
    String filePath = filePath = FileUtil.file(dir, fileName).getAbsolutePath();
    ;
    if (!FileUtil.exist(filePath)) {
      return fileName;
    }
    return findAvailableFilename(dir, name, ext, index + 1);
  }
  ...
  private FileItemResult fileToFileItem(File file, String folderPath) {
    ...
    if (fileItemResult.getType() == FileTypeEnum.FILE) {
      fileItemResult.setUrl(
          getDownloadUrl(StringUtils.concat(getCurrentUserBasePath(), folderPath, file.getName())));
    } else {
