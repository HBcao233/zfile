...
@Slf4j
public class Open115ServiceImpl extends AbstractProxyTransferService<Open115Param>
  ...
  private final Cache<String, String> DOWNLOAD_URL_CACHE =
      CacheUtil.newTimedCache(50 * 60 * 1000, 10 * 60 * 1000);
  ...
  public FileItemResult getFileItem(String pathAndName) {
    String fullPath =
        StringUtils.concat(param.getBasePath(), getCurrentUserBasePath(), pathAndName);
    ...
    JSONObject jsonObject =
        sendGetRequestWithAuth(
            ...
            new JSONObject().fluentPut("file_id", fileId));
  ...
  public boolean newFolder(String path, String name) {
    ...
    JSONObject jsonObject =
        sendPostRequestWithAuth(
            ...
            new JSONObject().fluentPut("pid", pathId).fluentPut("file_name", name));
  ...
  public boolean deleteFile(String path, String name) {
    String deleteFullPath =
        StringUtils.concat(param.getBasePath(), getCurrentUserBasePath(), path, name);
    ...
    sendPostRequestWithAuth(
        ...
        new JSONObject().fluentPut("parent_id", parentPathId).fluentPut("file_ids", fileId));
  ...
  public boolean deleteFolder(String path, String name) {
    ...
    sendPostRequestWithAuth(
        "https://proapi.115.com/open/ufile/delete",
        new JSONObject().fluentPut("parent_id", parentPathId).fluentPut("file_ids", pathId));
  ...
  public boolean copyFile(String path, String name, String targetPath, String targetName) {
    ...
    String targetFullPath =
        StringUtils.concat(param.getBasePath(), getCurrentUserBasePath(), targetPath);
  ...
  public boolean moveFile(String path, String name, String targetPath, String targetName) {
    String srcFullPath =
        StringUtils.concat(param.getBasePath(), getCurrentUserBasePath(), path, name);
    String targetFullPath =
        StringUtils.concat(param.getBasePath(), getCurrentUserBasePath(), targetPath, targetName);

    String srcFileId = idCacheService.getFileId(srcFullPath, true);
    String targetPathId =
        idCacheService.getPathId(
            StringUtils.concat(param.getBasePath(), getCurrentUserBasePath(), targetPath), true);

    // https://www.yuque.com/115yun/open/vc6fhi2mrkenmav2
    sendPostRequestWithAuth(
        "https://proapi.115.com/open/ufile/move",
        new JSONObject().fluentPut("to_cid", targetPathId).fluentPut("file_ids", srcFileId));
  ...
  public boolean moveFolder(String path, String name, String targetPath, String targetName) {
    ...
    sendPostRequestWithAuth(
        "https://proapi.115.com/open/ufile/move",
        new JSONObject().fluentPut("to_cid", targetPathId).fluentPut("file_ids", srcPathId));
  ...
  public boolean renameFile(String path, String name, String newName) {
    ...
    sendPostRequestWithAuth(
        ...
        new JSONObject().fluentPut("file_id", fileId).fluentPut("file_name", newName));

    idCacheService.deletePathId(fullPath);
    idCacheService.putFileId(
        StringUtils.concat(param.getBasePath(), getCurrentUserBasePath(), path, newName), fileId);
    return true;
  }
  ...
  public boolean renameFolder(String path, String name, String newName) {
    ...
    sendPostRequestWithAuth(
        "https://proapi.115.com/open/ufile/update",
        new JSONObject().fluentPut("file_id", pathId).fluentPut("file_name", newName));

    idCacheService.deletePathId(fullPath);
    idCacheService.putPathId(
        StringUtils.concat(param.getBasePath(), getCurrentUserBasePath(), path, newName), pathId);
    return true;
  }
  ...
  private String getProxyDownloadUrlByPickCode(String pathAndName, String pickCode) {
    if (param.isEnableProxyDownload() && StringUtils.isEmpty(param.getDomain())) {
      ...
      return StringUtils.concat(
          getSystemConfigService().getAxiosFromDomainOrSetting(),
          Open115UrlController.PROXY_DOWNLOAD_LINK_PREFIX,
          storageId + "",
          pickCode);
    }
  }
  ...
  public String getOpen115DownloadUrlByPickCode(String pickCode) {
    ...
    return DOWNLOAD_URL_CACHE.get(
        pickCode + "_" + userAgent,
        () -> {
  ...
  public void uploadFile(String pathAndName, InputStream inputStream, Long size) throws Exception {
    ...
    try {
      ...
      Open115UploadUtils.uploadFile(
          tempFile, fileName, pathId, this::checkExpiredAndGetAccessToken);
    } finally {
  ...
  public ResponseEntity<org.springframework.core.io.Resource> downloadToStream(String pathAndName)
    ...
    httpRequest.header(
        HttpHeaders.USER_AGENT, RequestHolder.getRequest().getHeader(HttpHeaders.USER_AGENT), true);
  ...
  public void refreshAccessToken() {
    try {
      ...
      RefreshTokenCacheBO.putRefreshTokenInfo(
          storageId,
          RefreshTokenCacheBO.RefreshTokenInfo.fail(
              getStorageTypeEnum().getDescription() + " AccessToken 刷新失败: " + e.getMessage()));
  ...
  private JSONObject sendRequest(
    ...
    HttpRequest httpRequest = HttpUtil.createRequest(method, url).header(headers, true).form(form);
  ...
  private static JSONObject handleEntity(HttpResponse httpResponse) {
    if (!httpResponse.isOk()) {
      throw new SystemException(
          "请求失败, 状态码: " + httpResponse.getStatus() + ", 响应体: " + httpResponse.body());
    }
    ...
    if (state == null || !state) {
      ...
      throw new SystemException(
          "请求失败, 响应消息: "
              ...
              + ", 响应码: "
              + code
              + ", 详见: https://www.yuque.com/115yun/open/rnq0cbz8tt7cu43i");
    }
  ...
  private FileItemResult listJsonToFileItem(JSONObject jsonObject, String folderPath) {
    ...
    if (Objects.equals(jsonObject.getString("fc"), FC_FILE)) {
      ...
      String fullPath =
          StringUtils.concat(
              param.getBasePath(), getCurrentUserBasePath(), folderPath, fileItemResult.getName());
      idCacheService.putFileId(fullPath, jsonObject.getString("fid"));

      String pickCode = jsonObject.getString("pc");
      fileItemResult.setUrl(
          getProxyDownloadUrlByPickCode(
              StringUtils.concat(getCurrentUserBasePath(), folderPath, fileItemResult.getName()),
              pickCode));
    } else {
