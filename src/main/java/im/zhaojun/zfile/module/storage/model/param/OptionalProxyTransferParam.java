...
public class OptionalProxyTransferParam extends ProxyTransferParam {
  ...
  @StorageParamItem(
      ...
      description = "启用该功能后，上传会先上传到服务器，完成后服务器再上传至目标存储源，强依赖服务器带宽大小，请确认必要后开启。",
      order = 100)
  ...
  @StorageParamItem(
      ...
      defaultValue = "false",
      type = StorageParamTypeEnum.SWITCH,
      description = "启用该功能后，下载会先下载到服务器，完成后服务器再下载返回给下载用户，强依赖服务器带宽大小，请确认必要后开启。",
      order = 101)
  private boolean enableProxyDownload;

  @StorageParamItem(
      name = "代理下载生成签名链接",
      condition = "enableProxyDownload==true",
      onlyOverwrite = {
        StorageParamItemAnnoEnum.NAME,
        StorageParamItemAnnoEnum.CONDITION,
        StorageParamItemAnnoEnum.ORDER
      },
      order = 102)
  ...
  @StorageParamItem(
      name = "代理下载签名有效期",
      condition = "proxyPrivate==true",
      required = false,
      defaultValue = "1800",
      description = "用于下载签名的有效期, 单位为秒, 如不配置则默认为 1800 秒.",
      order = 103)
  ...
  @StorageParamItem(
      ...
      type = StorageParamTypeEnum.SWITCH,
      defaultValue = "true",
      description = "控制代理下载时的下载行为：关闭则使用浏览器默认行为，启用则强制下载",
      order = 105)
