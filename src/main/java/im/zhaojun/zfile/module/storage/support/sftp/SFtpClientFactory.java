...
public class SFtpClientFactory extends BasePooledObjectFactory<Sftp> {
  ...
  public Sftp create() throws Exception {
    ...
    if (StringUtils.isBlank(privateKey)) {
      ...
      jsch.addIdentity(
          username, privateKey.getBytes(StandardCharsets.UTF_8), null, passphraseBytes);
    }
    Sftp sftp =
        new Sftp(
            session, charset, StorageSourceConnectionProperties.DEFAULT_CONNECTION_TIMEOUT_MILLIS);
