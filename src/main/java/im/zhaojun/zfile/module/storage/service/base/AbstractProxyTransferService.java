 ...
 * @author zhaojun
 */
public abstract class AbstractProxyTransferService<P extends ProxyTransferParam>
    extends AbstractBaseFileService<P> {
   ...
   *
   * @param pathAndName 文件路径及文件名称
   * @param useParamDomain 是否使用存储源参数中的域名替换系统配置中的域名作为下载地址
   * @return 默认的代理下载 URL
   */
  public String getProxyDownloadUrl(String pathAndName, boolean useParamDomain) {
    ...
    if (param.isProxyPrivate()) {
      urlBuilder.addQuery(
          "signature",
          ProxyDownloadUrlUtils.generatorSignature(
              storageId, pathAndName, param.getProxyTokenTime()));
    }
    ...
    if (!useParamDomain || StringUtils.isEmpty(param.getDomain())) {
      ...
      url =
          StringUtils.concat(
              domain,
              PROXY_DOWNLOAD_LINK_PREFIX,
              storageKey,
              StringUtils.encodeAllIgnoreSlashes(path));
    } else {
  ...
  public String getProxyUploadUrl(String path, String name) {
    ...
    String uploadUrl =
        StringUtils.concat(
            domain,
            PROXY_UPLOAD_LINK_PREFIX,
            storageKey,
            StringUtils.encodeAllIgnoreSlashes(fullPath));
   ...
   * 上传文件
   *
   * @param pathAndName 文件上传路径
   * @param inputStream 文件流
   * @param size 文件大小
   */
  public abstract void uploadFile(String pathAndName, InputStream inputStream, Long size)
      throws Exception;
   ...
   * @return 文件响应.
   */
  public abstract ResponseEntity<org.springframework.core.io.Resource> downloadToStream(
      String pathAndName) throws Exception;
