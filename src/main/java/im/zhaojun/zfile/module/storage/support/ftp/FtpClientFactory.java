...
public class FtpClientFactory extends BasePooledObjectFactory<Ftp> {
  ...
  public FtpClientFactory(
      String host, int port, String username, String password, Charset charset, String ftpMode) {
    this.host = host;
    this.port = port;
  ...
  public Ftp create() throws Exception {
    ...
    ftpConfig.setConnectionTimeout(
        StorageSourceConnectionProperties.DEFAULT_CONNECTION_TIMEOUT_MILLIS);
    Ftp ftp =
        new Ftp(
            ftpConfig,
            FtpServiceImpl.FTP_MODE_ACTIVE.equals(ftpMode) ? FtpMode.Active : FtpMode.Passive);
