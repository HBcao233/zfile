...
/**
 * 每个存储源对应一个 Service, 其中初始化好了与对象存储的配置信息. 此存储源上下文环境用户缓存每个 Service, 避免重复初始化. <br>
 *
 * @author zhaojun
 */
@Slf4j
public class StorageSourceContext {
  ...
  /** Map<Integer, AbstractBaseFileService> Map<存储源 ID, 存储源 Service> */
  private static final Map<Integer, AbstractBaseFileService<IStorageParam>> DRIVES_SERVICE_MAP =
      new ConcurrentHashMap<>();

  /** Map<String, Integer> Map<存储源 Key, 存储源 ID> */
  ...
  private static Map<StorageTypeEnum, AbstractBaseFileService> storageTypeEnumFileServiceMap =
      new HashMap<>();

  /** 缓存每个存储源参数的字段列表. */
  private static final Map<Class<?>, Map<String, Field>> PARAM_CLASS_FIELD_NAME_MAP_CACHE =
      new HashMap<>();
  ...
  public static AbstractBaseFileService<IStorageParam> getByStorageId(Integer storageId) {
    AbstractBaseFileService<IStorageParam> abstractBaseFileService =
        DRIVES_SERVICE_MAP.get(storageId);
   ...
   *
   * @param storageTypeEnum 存储源类型(枚举)
   * @return 存储源对应的 Service
   */
   ...
   * 根据存储类型获取对应的存储源的参数列表.
   *
   * @param type 存储类型: {@link StorageTypeEnum}
   * @return 指定类型存储源的参数列表. {@link
   *     StorageSourceSupport#getStorageSourceParamList(AbstractBaseFileService)} )}}
   */
   ...
   *
   * @param storageSourceInitDTO 存储源初始化对象
   */
  public static void init(StorageSourceInitDTO storageSourceInitDTO) {
    ...
    AbstractBaseFileService<IStorageParam> baseFileService =
        getInitStorageBeanByStorageType(storageTypeEnum);
    if (baseFileService == null) {
      throw new InvalidStorageSourceBizException(storageId);
    }

    // 填充初始化参数
    IStorageParam initParam =
        getInitParam(baseFileService, storageSourceInitDTO.getStorageSourceConfigList());
   ...
   *
   * @param storageTypeEnum 存储源类型
   * @return 存储源对应未初始化的 Service
   */
  private static AbstractBaseFileService<IStorageParam> getInitStorageBeanByStorageType(
  ...
  /** 获取指定存储源的初始化参数. */
  private static IStorageParam getInitParam(
      AbstractBaseFileService<?> baseFileService,
    ...
    if (!fieldMapCopy.isEmpty()) {
      List<StorageSourceParamDef> storageSourceParamList =
          StorageSourceSupport.getStorageSourceParamList(baseFileService);
      Map<String, StorageSourceParamDef> storageSourceParamDefMap =
          storageSourceParamList.stream()
  ...
  public static Map<Integer, RefreshTokenService> getAllRefreshTokenStorageSource() {
    ...
    for (Map.Entry<Integer, AbstractBaseFileService<IStorageParam>> baseFileServiceEntry :
        DRIVES_SERVICE_MAP.entrySet()) {
