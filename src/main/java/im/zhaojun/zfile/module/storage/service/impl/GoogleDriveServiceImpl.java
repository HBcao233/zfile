...
@Slf4j
public class GoogleDriveServiceImpl extends AbstractProxyTransferService<GoogleDriveParam>
  ...
  /** 文件上传操作 API */
  private static final String DRIVE_FILE_UPLOAD_URL =
      "https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart";

  /** 刷新 AccessToken URL */
  private static final String REFRESH_TOKEN_URL = "https://oauth2.googleapis.com/token";

  @jakarta.annotation.Resource private StorageSourceConfigService storageSourceConfigService;
  ...
  private String getIdByPath(String path, boolean concatCurrentUserBasePath) {
    String fullPath =
        StringUtils.concat(
            param.getBasePath(), concatCurrentUserBasePath ? getCurrentUserBasePath() : "", path);
    ...
    for (String subPath : pathList) {
      ...
      HttpRequest httpRequest =
          commonHttpRequest(HttpUtil.createGet(DRIVE_FILE_URL + "?" + folderIdParam));
  ...
  public List<FileItemResult> fileList(String folderPath) throws Exception {
    ...
    do {
      ...
      HttpRequest httpRequest =
          commonHttpRequest(HttpUtil.createGet(DRIVE_FILE_URL + "?" + folderIdParam));
      httpRequest.setConnectionTimeout(
          StorageSourceConnectionProperties.DEFAULT_CONNECTION_TIMEOUT_MILLIS);
  ...
  public FileItemResult getFileItem(String pathAndName) {
    ...
    HttpRequest httpRequest =
        commonHttpRequest(HttpUtil.createGet(DRIVE_FILE_URL + StringUtils.SLASH + fileId));
  ...
  public boolean newFolder(String path, String name) {
    HttpResponse httpResponse =
        commonHttpRequest(HttpUtil.createPost(DRIVE_FILE_URL))
            .body(
                new JSONObject()
                    .fluentPut("name", name)
    ...
    HttpResponse httpResponse =
        commonHttpRequest(
                HttpUtil.createRequest(
                    Method.DELETE, DRIVE_FILE_URL + StringUtils.SLASH + getIdByPath(pathAndName)))
            .execute();
  ...
  public boolean renameFile(String path, String name, String newName) {
    ...
    HttpResponse httpResponse =
        commonHttpRequest(
                HttpUtil.createRequest(Method.PATCH, DRIVE_FILE_URL + StringUtils.SLASH + fileId))
            .body(new JSONObject().fluentPut("name", newName).toJSONString())
            .execute();
  ...
  public void uploadFile(String pathAndName, InputStream inputStream, Long size) {
    ...
    try (CloseableHttpClient httpClient = HttpClients.createDefault()) {
      HttpUriRequest httpUriRequest =
          RequestBuilder.post(DRIVE_FILE_UPLOAD_URL)
  ...
  public boolean copyFile(String path, String name, String targetPath, String targetName) {
    ...
    HttpResponse httpResponse =
        commonHttpRequest(
                HttpUtil.createPost(DRIVE_FILE_URL + StringUtils.SLASH + srcFileId + "/copy"))
            .body(
                new JSONObject()
   ...
   *
   * @param jsonArray api 返回文件 json array
   ...
   *
   * @param jsonObject api 返回文件 json object
   * @param folderPath 所属文件夹路径
   * @return zfile 文件对象
   */
  ...
  class GoogleDriveAPIParam {
     ...
     * 根据路径获取 id 的 api 请求参数
     *
     * @param folderId google drive 文件夹 id
     * @param pageToken 分页 token
     */
    public String getFileListParam(String folderId, String pageToken) {
      GoogleDriveAPIParam googleDriveAPIParam = getBasicParam();

      googleDriveAPIParam.setFields(
          "files(id,name,mimeType,shortcutDetails,size,modifiedTime),nextPageToken");
    ...
    public String getSearchParam(String folderId, String pageToken, String keyword) {
      ...
      googleDriveAPIParam.setFields(
          "files(id,name,mimeType,shortcutDetails,size,modifiedTime),nextPageToken");
      googleDriveAPIParam.setQ(
          parentIdParam + " name contains '" + keyword + "' and trashed = false");
      googleDriveAPIParam.setPageToken(pageToken);
      googleDriveAPIParam.setPageSize(DEFAULT_PAGE_SIZE);
      return googleDriveAPIParam.toString();
    }
   ...
   *
   * @param closeableHttpResponse http 响应
   */
  private void checkHttpResponseIsError(CloseableHttpResponse closeableHttpResponse)
      throws IOException {
