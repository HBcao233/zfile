...
public class GoogleDriveParam extends ProxyTransferParam {
  ...
  @StorageParamItem(
      name = "clientId",
      defaultValue = "${zfile.gd.clientId}",
      order = 1,
      description = "<font color=\"red\">默认 API 仅用作示例，因审核原因，目前不可用，请自行申请 API</font>",
      link = "https://docs.zfile.vip/advanced#google-drive-api",
      linkName = "自定义 API 文档")
  ...
  @StorageParamItem(
      name = "回调地址",
      description = "这里要修改为自己的域名",
      defaultValue = "${zfile.gd.redirectUri}",
      order = 3)
  ...
  @StorageParamItem(
      name = "加速域名",
      required = false,
      description = "可使用 cf worker index 程序的链接，会使用 cf 中转下载，教程自行查询. 不填写则使用服务器中转下载.")
