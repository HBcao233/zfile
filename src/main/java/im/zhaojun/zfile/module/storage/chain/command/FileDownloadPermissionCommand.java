...
/**
 * 检查是否有权限下载 command 命令 检查是否有权限下载和预览文件，如果都没有权限，则去除返回结果中的下载地址
 *
 * @author zhaojun
 */
@Service
  ...
  public boolean execute(Context context) throws Exception {
    ...
    boolean hasDownloadPermission =
        userStorageSourceService.hasCurrentUserStorageOperatorPermission(
            storageId, FileOperatorTypeEnum.DOWNLOAD);
    boolean hasPreviewPermission =
        userStorageSourceService.hasCurrentUserStorageOperatorPermission(
            storageId, FileOperatorTypeEnum.PREVIEW);
