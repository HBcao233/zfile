...
public class GoogleDriveOAuth2ServiceImpl implements IOAuth2Service {
  ...
  public String generateAuthorizationUrl(String clientId, String clientSecret, String redirectUri) {
    ...
    String stateStr =
        "&state="
            + Base64.encodeUrlSafe(StringUtils.join("::", clientId, clientSecret, redirectUri));
    ...
    return "https://accounts.google.com/o/oauth2/v2/auth?client_id="
        ...
        + redirectUri
        + "&scope="
        + zFileProperties.getGd().getScope()
        + "&access_type=offline"
        + "&prompt=consent"
        + stateStr;
  }
  ...
  @Override
  public OAuth2TokenDTO getTokenByCode(
    ...
    ResponseEntity<String> response =
        new RestTemplate(new NoRedirectClientHttpRequestFactory())
    ...
    return OAuth2TokenDTO.success(
        clientId, clientSecret, redirectUri, accessToken, refreshToken, responseBody, expiresIn);
  }
  ...
  private static class NoRedirectClientHttpRequestFactory extends SimpleClientHttpRequestFactory {
    ...
    protected void prepareConnection(
        @NotNull HttpURLConnection connection, @NotNull String httpMethod) throws IOException {
