...
public class OneDriveCallbackController {
  ...
  @Resource private OneDriveOAuth2ServiceImpl oneDriveOAuth2Service;

  @Resource private OneDriveChinaOAuth2ServiceImpl oneDriveChinaOAuth2Service;

  @GetMapping("/authorize")
  @ApiOperationSupport(order = 1)
  @Operation(
      summary = "生成 OAuth2 登陆 URL",
      description = "生成 OneDrive OAuth2 登陆 URL，用户国际版，家庭版等非世纪互联运营的 OneDrive.")
  public String authorize(String clientId, String clientSecret, String redirectUri) {
    String authorizeUrl =
        oneDriveOAuth2Service.generateAuthorizationUrl(clientId, clientSecret, redirectUri);
  ...
  @Operation(
      summary = "OAuth2 回调地址",
      description =
          "根据 OAuth2 协议，登录成功后，会返回给网站一个 code，用此 code 去换取 accessToken 和 refreshToken.（oneDrive"
              + " 会回调此接口）")
  public String oneDriveCallback(String code, String state, Model model) {
    ...
    OAuth2TokenDTO oAuth2TokenDTO =
        oneDriveOAuth2Service.getTokenByCode(code, clientId, clientSecret, redirectUri);
  ...
  @Operation(
      summary = "生成 OAuth2 登陆 URL(世纪互联)",
      description = "生成 OneDrive OAuth2 登陆 URL，用于世纪互联版本.")
  public String authorizeChina(String clientId, String clientSecret, String redirectUri) {
    String authorizeUrl =
        oneDriveChinaOAuth2Service.generateAuthorizationUrl(clientId, clientSecret, redirectUri);
    return "redirect:" + authorizeUrl;
  }
  ...
  public String oneDriveChinaCallback(String code, String state, Model model) {
    ...
    OAuth2TokenDTO OAuth2TokenDTO =
        oneDriveChinaOAuth2Service.getTokenByCode(code, clientId, clientSecret, redirectUri);
    model.addAttribute("oauth2Token", OAuth2TokenDTO);
