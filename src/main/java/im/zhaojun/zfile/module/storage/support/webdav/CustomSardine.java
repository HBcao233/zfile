...
public class CustomSardine extends SardineImpl {
  ...
  protected <T> T execute(
      HttpClientContext context, HttpRequestBase request, ResponseHandler<T> responseHandler)
      throws IOException {
    ...
    if (connectTimeout != null && connectTimeout.compareTo(Duration.ZERO) > 0) {
      configBuilder.setConnectTimeout(
          (int) (1000 * connectTimeout.getSeconds() + connectTimeout.getNano() / 1000000));
    ...
    if (readTimeout != null && readTimeout.compareTo(Duration.ZERO) > 0) {
      configBuilder.setSocketTimeout(
          (int) (1000 * readTimeout.getSeconds() + readTimeout.getNano() / 1000000));
    }
