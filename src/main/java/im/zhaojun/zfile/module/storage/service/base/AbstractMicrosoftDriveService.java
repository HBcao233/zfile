 ...
 */
@Slf4j
public abstract class AbstractMicrosoftDriveService<P extends MicrosoftDriveParam>
    extends AbstractProxyTransferService<P> implements RefreshTokenService {

  @Resource private StorageSourceConfigService storageSourceConfigService;

  /** 获取根文件 API URI */
  protected static final String DRIVER_ROOT_URL =
      "https://{graphEndPoint}/v1.0/{type}/drive/root/children?select=name,size,lastModifiedDateTime,file,@microsoft.graph.downloadUrl,@odata.nextLink,value";

  /** 获取非根文件 API URI */
  protected static final String DRIVER_ITEMS_URL =
      "https://{graphEndPoint}/v1.0/{type}/drive/root:{path}:/children?select=name,size,lastModifiedDateTime,file,@microsoft.graph.downloadUrl,@odata.nextLink,value";

  /** 获取单文件 API URI */
  protected static final String DRIVER_ITEM_URL =
      "https://{graphEndPoint}/v1.0/{type}/drive/root:{path}?select=name,size,lastModifiedDateTime,file,@microsoft.graph.downloadUrl,id";

  /** 操作单文件 API URI */
  protected static final String DRIVER_ITEM_OPERATOR_URL =
      "https://{graphEndPoint}/v1.0/{type}/drive/root:{path}";

  /** 根据 RefreshToken 获取 AccessToken API URI */
  protected static final String AUTHENTICATE_URL =
      "https://{authenticateEndPoint}/common/oauth2/v2.0/token";

  /** 创建上传文件回话 API */
  protected static final String CREATE_UPLOAD_SESSION_URL =
      "https://{graphEndPoint}/v1.0/{type}/drive/root:{path}:/createUploadSession";

  /** 复制文件 API */
  private static final String DRIVER_COPY_URL =
      "https://{graphEndPoint}/v1.0/{type}/drive/root:{path}:/copy";
  ...
  public void init() {
    ...
    if (refreshTokenExpiredAt == null) {
      ...
      RefreshTokenInfoDTO tokenInfoDTO =
          RefreshTokenInfoDTO.success(
              param.getAccessToken(), param.getRefreshToken(), refreshTokenExpiredAt);
      RefreshTokenCacheBO.putRefreshTokenInfo(
          storageId, RefreshTokenCacheBO.RefreshTokenInfo.success(tokenInfoDTO));
    }
  }
    ...
    if (restTemplate == null) {
      synchronized (this) {
        if (restTemplate == null) {
          ...
          int timeoutSecond =
              param.getProxyUploadTimeoutSecond() == null ? 0 : param.getProxyUploadTimeoutSecond();
          RequestConfig requestConfig =
              RequestConfig.custom().setResponseTimeout(Timeout.ofSeconds(timeoutSecond)).build();
  ...
  public List<FileItemResult> fileList(String folderPath) {
    String fullPath =
        StringUtils.concatTrimEndSlashes(param.getBasePath(), getCurrentUserBasePath(), folderPath);
    ...
    do {
      ...
      for (int i = 0; i < fileList.size(); i++) {
        ...
        if (param.isEnableProxyDownload() && StringUtils.isEmpty(param.getProxyDomain())) {
          fileItemResult.setUrl(
              getProxyDownloadUrl(
                  StringUtils.concat(
                      getCurrentUserBasePath(), folderPath, fileItemResult.getName())));
        }
  ...
  @Nullable private JSONObject getFileOriginInfo(String pathAndName) {
  ...
  public boolean newFolder(String path, String name) {
    ...
    String fullPath =
        StringUtils.concatTrimEndSlashes(param.getBasePath(), getCurrentUserBasePath(), path);
  ...
  public void uploadFile(String pathAndName, InputStream inputStream, Long size)
    ...
    try {
      getRestTemplate()
          .execute(
              ...
              request -> {
                ...
                headers.set(
                    HttpHeaders.CONTENT_RANGE, "bytes 0-" + (size - 1) + StringUtils.SLASH + size);
                StreamUtils.copy(inputStream, request.getBody());
              },
              clientHttpResponse -> {
                if (!clientHttpResponse.getStatusCode().is2xxSuccessful()) {
                  throw new UploadFileFailSystemException(
                      this.getStorageTypeEnum(),
                      pathAndName,
                      size,
      ...
      if (e instanceof ResourceAccessException
          && e.getMessage() != null
          && e.getMessage().contains("Timeout on")) {
        throw new BizException(ErrorCode.BIZ_UPLOAD_FILE_TIMEOUT_ERROR);
      }
      throw new UploadFileFailSystemException(
          this.getStorageTypeEnum(), pathAndName, size, 500, e.getMessage(), e);
    }
  }
  ...
  @Override
  public ResponseEntity<org.springframework.core.io.Resource> downloadToStream(String pathAndName)
      throws IOException {
        ...
        .execute(
            ...
            clientHttpResponse -> {
              ...
              RequestHolder.writeFile(
                  inputStream, fileName, fileSize, false, param.isProxyLinkForceDownload());
              return null;
            });
    return null;
  }
    ...
    try {
      ...
      StorageSourceConfig accessTokenConfig =
          storageSourceConfigService.findByStorageIdAndName(
              storageId, StorageConfigConstant.ACCESS_TOKEN_KEY);
      StorageSourceConfig refreshTokenConfig =
          storageSourceConfigService.findByStorageIdAndName(
              storageId, StorageConfigConstant.REFRESH_TOKEN_KEY);
      StorageSourceConfig refreshTokenExpiredAtConfig =
          storageSourceConfigService.findByStorageIdAndName(
              storageId, StorageConfigConstant.REFRESH_TOKEN_EXPIRED_AT_KEY);
      ...
      storageSourceConfigService.updateBatch(
          storageId,
          Arrays.asList(accessTokenConfig, refreshTokenConfig, refreshTokenExpiredAtConfig));
      RefreshTokenCacheBO.putRefreshTokenInfo(
          storageId, RefreshTokenCacheBO.RefreshTokenInfo.success(tokenInfoDTO));
    } catch (Exception e) {
      RefreshTokenCacheBO.putRefreshTokenInfo(
          storageId,
          RefreshTokenCacheBO.RefreshTokenInfo.fail("AccessToken 刷新失败: " + e.getMessage()));
  ...
  private RefreshTokenInfoDTO getAndRefreshToken() {
    ...
    String param =
        "client_id="
            ...
            + getClientSecret()
            + "&refresh_token="
            + refreshStorageSourceConfig.getValue()
    ...
    String authenticateUrl =
        AUTHENTICATE_URL.replace("{authenticateEndPoint}", getAuthenticateEndPoint());
    HttpResponse response =
        HttpUtil.createPost(authenticateUrl)
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          "{} 刷新令牌完成. 响应状态码: {}, 响应体: {}", getStorageSimpleInfo(), responseStatus, responseBody);
    }
  ...
  private String checkExpiredAndGetAccessToken() {
    RefreshTokenCacheBO.RefreshTokenInfo refreshTokenInfo =
        RefreshTokenCacheBO.getRefreshTokenInfo(storageId);
