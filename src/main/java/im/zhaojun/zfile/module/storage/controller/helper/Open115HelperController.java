...
public class Open115HelperController {
  ...
  public AjaxJson<Open115AuthDeviceCodeResult> generateQrCode(String appId) {
    ...
    HttpRequest httpRequest =
        HttpUtil.createPost("https://passportapi.115.com/open/authDeviceCode")
    ...
    Open115AuthDeviceCodeResult open115AuthDeviceCodeResult =
        JSON.parseObject(body).getObject("data", Open115AuthDeviceCodeResult.class);
  ...
  public AjaxJson<Open115GetStatusResult> getQrCodeStatus(
      @RequestBody Open115AuthDeviceCodeResult open115AuthDeviceCodeResult) {

    // https://www.yuque.com/115yun/open/shtpzfhewv5nag11#6d33298a
    HttpRequest httpRequest =
        HttpUtil.createGet("https://qrcodeapi.115.com/get/status/")
    ...
    if (jsonObject.getInteger("state") == 1
        && !jsonObject.getJSONObject("data").containsKey("status")) {
      return AjaxJson.getSuccessData(Open115GetStatusResult.waiting());
    }

    if (jsonObject.getInteger("state") == 1
        && jsonObject.getJSONObject("data").getInteger("status") == 1) {
      return AjaxJson.getSuccessData(
          Open115GetStatusResult.scanning(jsonObject.getJSONObject("data").getString("msg")));
    }

    // https://www.yuque.com/115yun/open/shtpzfhewv5nag11#QCCVQ
    HttpRequest deviceCodeToTokenHttpRequest =
        HttpUtil.createPost("https://passportapi.115.com/open/deviceCodeToToken")
            .form("uid", open115AuthDeviceCodeResult.getUid())
    ...
    JSONObject deviceCodeToTokenJsonObject =
        JSON.parseObject(deviceCodeToTokenBody).getJSONObject("data");
    ...
    return AjaxJson.getSuccessData(
        Open115GetStatusResult.success(accessToken, refreshToken, expiredAt));
  }
}
