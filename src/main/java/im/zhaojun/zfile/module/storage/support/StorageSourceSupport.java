...
public class StorageSourceSupport {
  ...
  /** 存储源类型与存储源参数列表的缓存 */
  private static final Map<Class<? extends AbstractBaseFileService>, List<StorageSourceParamDef>>
      STORAGE_SOURCE_PARAM_CACHE = new ConcurrentHashMap<>();
   ...
   * @return 存储源参数列表定义
   */
  public static List<StorageSourceParamDef> getStorageSourceParamList(
      AbstractBaseFileService abstractBaseFileService) {
    Class<? extends AbstractBaseFileService> clazz;
    if (AopUtils.isAopProxy(abstractBaseFileService)) {
      clazz =
          (Class<? extends AbstractBaseFileService>)
              AopUtils.getTargetClass(abstractBaseFileService);
    } else {
    ...
    for (Field field : fields) {
      ...
      String defaultValue =
          PlaceholderUtils.resolvePlaceholdersBySpringProperties(
              storageParamItemAnnotation.defaultValue());
      ...
      List<StorageSourceParamDef.Options> optionsList =
          getOptionsList(storageParamItemAnnotation, storageParam);
      ...
      StorageSourceParamDef storageSourceParamDef =
          storageSourceParamDefMap.getOrDefault(fieldName, new StorageSourceParamDef());
      ...
      if (fieldOverrideSetIsEmpty
          || !fieldOverrideSet.contains(StorageParamItemAnnoEnum.DESCRIPTION)) {
        storageSourceParamDef.setDescription(description);
      }
      if (fieldOverrideSetIsEmpty
          || !fieldOverrideSet.contains(StorageParamItemAnnoEnum.REQUIRED)) {
        storageSourceParamDef.setRequired(required);
      }
      if (fieldOverrideSetIsEmpty
          || !fieldOverrideSet.contains(StorageParamItemAnnoEnum.DEFAULT_VALUE)) {
      ...
      if (fieldOverrideSetIsEmpty
          || !fieldOverrideSet.contains(StorageParamItemAnnoEnum.LINK_NAME)) {
      ...
      if (fieldOverrideSetIsEmpty
          || !fieldOverrideSet.contains(StorageParamItemAnnoEnum.OPTION_ALLOW_CREATE)) {
      ...
      if (fieldOverrideSetIsEmpty
          || !fieldOverrideSet.contains(StorageParamItemAnnoEnum.CONDITION)) {
      ...
      StorageParamItemAnnoEnum[] storageParamItemAnnoEnumArray =
          storageParamItemAnnotation.onlyOverwrite();
      if (ArrayUtils.isNotEmpty(storageParamItemAnnoEnumArray)) {
        Set<StorageParamItemAnnoEnum> set =
            fieldOverrideMap.getOrDefault(fieldName, new HashSet<>());
   ...
   * 从注解中获取 options 列表
   *
   * @param storageParamItemAnnotation 存储源参数注解
   * @return options 列表，如果没有则返回空列表，不会返回 null
   */
  private static List<StorageSourceParamDef.Options> getOptionsList(
      StorageParamItem storageParamItemAnnotation, IStorageParam storageParam) {
    // 如果不是默认的空接口实现，优先从实现类中通过反射获取 options 列表
    Class<? extends StorageParamSelect> storageParamSelectClass =
        storageParamItemAnnotation.optionsClass();
    if (BooleanUtils.isNotTrue(storageParamSelectClass.isInterface())) {
      ...
      List<StorageSourceParamDef.Options> options =
          storageParamSelect.getOptions(storageParamItemAnnotation, storageParam);
    ...
    if (ArrayUtils.isNotEmpty(options)) {
      for (StorageParamSelectOption storageParamSelectOption : options) {
        StorageSourceParamDef.Options option =
            new StorageSourceParamDef.Options(storageParamSelectOption);
