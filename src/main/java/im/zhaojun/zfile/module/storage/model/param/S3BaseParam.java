...
public class S3BaseParam extends OptionalProxyTransferParam {
  ...
  @StorageParamItem(
      name = "区域",
      order = 30,
      description = "如下拉列表中没有的区域，或想使用内网地址，可直接输入后回车，如: xxx-cn-beijing.example.com")
  private String endPoint;

  @StorageParamItem(
      name = "EndPoint 协议",
      order = 31,
      description = "指定 EndPoint 使用的协议, 默认为 http",
  ...
  @StorageParamItem(
      ...
      required = false,
      defaultValue = "1800",
      description = "当为私有空间时, 用于下载签名的有效期, 单位为秒, 如不配置则默认为 1800 秒.")
