...
public class MicrosoftDriveParam extends OptionalProxyTransferParam {
  ...
  @StorageParamItem(
      name = "回调地址",
      description = "如使用自定义 api, 需将此处默认的域名修改为您的域名, 且需在 api 中配置为回调域名.",
      defaultValue = "${zfile.onedrive.redirectUri}",
      order = 3)
  ...
  @StorageParamItem(
      ...
      required = false,
      order = 7,
      description = "使用 CF 或自建反代下载地址时填写，否则请留空。")
  ...
  @StorageParamItem(
      name = "代理上传超时时间",
      condition = "enableProxyUpload==true",
      defaultValue = "300",
      description = "服务器代理上传至微软云的超时时间, 单位为秒, 默认为 300 秒. 请自行根据服务器带宽大小、上传文件调整，为 0 则不限制.",
      order = 101)
  private Integer proxyUploadTimeoutSecond;

  @StorageParamItem(
      condition = "proxyDomain==",
      onlyOverwrite = {StorageParamItemAnnoEnum.CONDITION, StorageParamItemAnnoEnum.ORDER},
      order = 101)
