...
public class UpYunServiceImpl extends AbstractProxyTransferService<UpYunParam> {
  ...
  public List<FileItemResult> fileList(String folderPath) throws Exception {
    ...
    do {
      ...
      UpYun.FolderItemIter folderItemIter =
          upYun.readDirIter(
              StringUtils.concat(true, param.getBasePath(), getCurrentUserBasePath(), folderPath),
              hashMap);
      ...
      if (folderItems != null) {
        for (UpYun.FolderItem folderItem : folderItems) {
          ...
          fileItemResult.setType(
              "folder".equals(folderItem.type) ? FileTypeEnum.FOLDER : FileTypeEnum.FILE);
  ...
  public String getOriginDownloadUrl(String pathAndName) {
    ...
    String domain =
        StringUtils.isBlank(param.getDomain())
            ? "http://" + param.getBucketName() + ".test.upcdn.net"
            : param.getDomain();

    String baseDownloadUrl =
        StringUtils.concat(domain, StringUtils.encodeAllIgnoreSlashes(fullPath));
    // 判断是否配置了 token 防盗链.
    if (StringUtils.isNotEmpty(param.getToken())) {
      ...
      String downloadToken =
          SecureUtil.md5(param.getToken() + "&" + etime + "&" + fullPath).substring(12, 20);
  ...
  public FileItemResult getFileItem(String pathAndName) {
    String encodeFullUrl =
        StringUtils.concat(true, param.getBasePath(), getCurrentUserBasePath(), pathAndName);
  ...
  public boolean newFolder(String path, String name) {
    String fullPath =
        StringUtils.concat(true, param.getBasePath(), getCurrentUserBasePath(), path, name);
    try {
  ...
  public boolean renameFile(String path, String name, String newName) {
    String srcPath =
        StringUtils.concat(
            true, param.getBucketName(), param.getBasePath(), getCurrentUserBasePath(), path, name);
    String distPath =
        StringUtils.concat(true, param.getBasePath(), getCurrentUserBasePath(), path, newName);
  ...
  public String getUploadUrl(String path, String name, Long size) {
    ...
    uploadSignParam.setPath(
        StringUtils.concat(param.getBasePath(), getCurrentUserBasePath(), path));
  ...
  public boolean copyFile(String path, String name, String targetPath, String targetName) {
    ...
    String distPath =
        StringUtils.concat(
            true, param.getBasePath(), getCurrentUserBasePath(), targetPath, targetName);
  ...
  public AuthModel generatorAuthModel(UploadSignParam uploadSignParam) {
    ...
    // 上传，处理，内容识别有存储
    String signature =
        sign(param.getUsername(), SecureUtil.md5(param.getPassword()), method, uri, policy);
  ...
  private String getPolicy(UploadSignParam uploadSignParam) {
    ...
    params.put(
        Params.SAVE_KEY, StringUtils.concat(uploadSignParam.getPath(), uploadSignParam.getName()));
  ...
  private void tryUpload(String pathAndName, InputStream inputStream)
      throws IOException, UpException {
    String encodeFullUrl =
        StringUtils.concat(true, param.getBasePath(), getCurrentUserBasePath(), pathAndName);
