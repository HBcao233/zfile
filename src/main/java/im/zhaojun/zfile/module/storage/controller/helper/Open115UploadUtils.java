...
public class Open115UploadUtils {
  ...
  private static final Cache<String, UploadTokenResponse> UPLOAD_TOKEN_CACHE =
      CacheUtil.newTimedCache(55 * 60 * 1000);

  /**
   * 上传文件到 115，自动适应秒传，非秒传场景.
   *
   * @param file 要上传的文件
   * @param targetDirId 目标文件夹 ID，0 代表根目录
   * @param accessTokenSupplier 115 OPEN API 的访问令牌提供者，通常是一个 lambda 表达式或方法引用，这里使用 supplier
   *     是为了防止上传过程中访问令牌过期导致的错误。
   * @return 文件上传成功后返回的 pick_code，如果是秒传则返回对应的 pick_code。
   */
  public static String uploadFile(
      File file, String fileName, String targetDirId, Supplier<String> accessTokenSupplier)
      throws Exception {
    InitUploadResponse initResponse =
        initUploadWithAuthHandling(file, fileName, targetDirId, accessTokenSupplier);
  ...
  private static InitUploadResponse initUploadWithAuthHandling(
      File file, String fileName, String targetDirId, Supplier<String> accessToken)
      throws Exception {
    ...
    while (true) {
      ...
      HttpResponse response =
          HttpRequest.post(BASE_URL + INIT_UPLOAD_PATH)
  ...
  private static void uploadToObjectStorage(
      File file, InitUploadResponse.Data initData, UploadTokenResponse.Data tokenData)
      throws Exception {
  ...
  private static UploadTokenResponse getUploadToken(Supplier<String> accessToken) {
    ...
    return UPLOAD_TOKEN_CACHE.get(
        accessTokenStr,
        false,
        (Func0<UploadTokenResponse>)
            () -> {
              // https://www.yuque.com/115yun/open/ul4mrauo5i2uza0q
              HttpResponse response =
                  HttpRequest.get(BASE_URL + GET_TOKEN_PATH).bearerAuth(accessTokenStr).execute();

              String responseBody = response.body();
              UploadTokenResponse tokenResponse =
                  JSON.parseObject(responseBody, UploadTokenResponse.class);
  ...
  static class OssHeaderInterceptor implements ExecutionInterceptor {
    ...
    public SdkHttpRequest modifyHttpRequest(
      ...
      Optional<String> callbackVarHeader =
          request.firstMatchingHeader("x-amz-meta-x-oss-callback-var");
