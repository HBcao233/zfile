...
public class SharePointHelperController {
  ...
  public AjaxJson<List<SharepointSiteResult>> getSites(
      @Valid @RequestBody SharePointSearchSitesRequest searchSitesRequest) {
    ...
    if (execute.getStatus() != HttpStatus.OK.value()) {
      throw new APIHttpRequestBizException(
          ErrorCode.BIZ_AUTO_GET_SHARE_POINT_SITES_ERROR, requestUrl, execute.getStatus(), body);
    }
    ...
    for (int i = 0; i < valueArray.size(); i++) {
      SharepointSiteResult sharepointSiteResult =
          valueArray.getObject(i, SharepointSiteResult.class);
  ...
  public AjaxJson<List<SharepointSiteListResult>> getSites(
      @Valid @RequestBody SharePointSiteListsRequest sharePointSiteListsRequest) {
    ...
    JSONArray valueArray = rootObject.getJSONArray("value");
    for (int i = 0; i < valueArray.size(); i++) {
      SharepointSiteListResult sharepointSiteListResult =
          valueArray.getObject(i, SharepointSiteListResult.class);
      // 如果是事件目录，则跳过
      ...
      // 如果是文档类型，则改名为"默认文档"
      if (Objects.equals(
          SHAREPOINT_LIST_TYPE_DOCUMENT, sharepointSiteListResult.getDisplayName())) {
  ...
  @ResponseBody
  public AjaxJson<String> getDomainPrefix(
      @RequestBody SharePointInfoRequest sharePointInfoRequest) {
    // 请求 URL
  ...
  public AjaxJson<String> getSiteId(@RequestBody SharePointInfoRequest sharePointInfoRequest) {
    ...
    // 判断必填参数
    if (sharePointInfoRequest == null
        || sharePointInfoRequest.getAccessToken() == null
        || sharePointInfoRequest.getSiteName() == null) {
    ...
    // 如果没有域名前缀, 则先获取
    if (sharePointInfoRequest.getDomainPrefix() == null
        || sharePointInfoRequest.getDomainType() == null) {
      String requestUrl = getSiteRootUrlByType(sharePointInfoRequest.getType());
      HttpRequest getRequest = HttpUtil.createGet(requestUrl);
    ...
    String requestUrl =
        String.format(
            "https://%s/v1.0/sites/%s.sharepoint.%s:/%s/%s",
            host,
   ...
   * 根据类型获取 API 地址
   *
   * @param type 网站类型： Standard：标准版 China：世纪互联版
   * @return API 地址
   */
