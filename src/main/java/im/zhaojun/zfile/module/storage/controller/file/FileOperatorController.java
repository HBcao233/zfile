...
import im.zhaojun.zfile.module.storage.model.enums.CharsetEnum;
...
public class FileOperatorController {
  ...
  public AjaxJson<Void> mkdir(@Valid @RequestBody NewFolderRequest newFolderRequest) {
    AbstractBaseFileService<?> fileService =
        StorageSourceContext.getByStorageKey(newFolderRequest.getStorageKey());
  ...
  @DemoDisable
  public AjaxJson<List<BatchOperatorResult>> deleteFile(
      @Valid @RequestBody BatchDeleteRequest batchDeleteRequest) {
    AbstractBaseFileService<?> fileService =
        StorageSourceContext.getByStorageKey(batchDeleteRequest.getStorageKey());
    if (fileService == null) {
      ...
      if (pathCheckResult == null) {
        ...
        VerifyResultDTO verifyResultDTO =
            passwordConfigService.verifyPassword(
                fileService.getStorageId(), fullPath, deleteItem.getPassword());
        // 校验不通过, 则跳过此删除的文件
        if (!verifyResultDTO.isPassed()) {
          log.warn(
              "因密码原因删除失败, 类型: {}, 路径: {}, 名称: {}, 原因: {}",
              deleteType,
              deletePath,
              deleteName,
              verifyResultDTO.getErrorCode());
  ...
  public AjaxJson<Void> rename(@Valid @RequestBody RenameFileRequest renameFileRequest) {
    AbstractBaseFileService<?> fileService =
        StorageSourceContext.getByStorageKey(renameFileRequest.getStorageKey());
    boolean flag =
        fileService.renameFile(
            renameFileRequest.getPath(),
            renameFileRequest.getName(),
            renameFileRequest.getNewName());
    if (flag) {
  ...
  public AjaxJson<Void> renameFolder(@Valid @RequestBody RenameFolderRequest renameFolderRequest) {
    AbstractBaseFileService<?> fileService =
        StorageSourceContext.getByStorageKey(renameFolderRequest.getStorageKey());
    boolean flag =
        fileService.renameFolder(
            renameFolderRequest.getPath(),
            renameFolderRequest.getName(),
  ...
  @CheckPassword(
      ...
      passwordFieldExpression = "[0].password")
  @DemoDisable
  public AjaxJson<String> getUploadFileUrl(
      @Valid @RequestBody UploadFileRequest uploadFileRequest) {
    AbstractBaseFileService<?> fileService =
        StorageSourceContext.getByStorageKey(uploadFileRequest.getStorageKey());
    String uploadUrl =
        fileService.getUploadUrl(
            uploadFileRequest.getPath(), uploadFileRequest.getName(), uploadFileRequest.getSize());
  ...
  @DemoDisable
  public AjaxJson<List<BatchOperatorResult>> moveFile(
      @Valid @RequestBody BatchMoveOrCopyFileRequest batchMoveOrCopyFileRequest,
      @PathVariable("action") String action,
      @PathVariable("type") String type) {
    if (batchMoveOrCopyFileRequest.getNameList().size()
        != batchMoveOrCopyFileRequest.getTargetNameList().size()) {
    ...
    for (int i = 0; i < targetNameList.size(); i++) {
      ...
      if (targetPath.startsWith(srcFullPath)) {
        batchOperatorResults.add(
            BatchOperatorResult.fail(
                srcPath, srcName, "不能" + dictMap.get(action) + dictMap.get(type) + "到自己的子文件夹下"));
        continue;
      }
      ...
      try {
        ...
        if (e instanceof BizException bizException) {
          if (!Objects.equals(
              bizException.getCode(), ErrorCode.BIZ_UNSUPPORTED_OPERATION.getCode())) {
            log.warn(
                "批量{}{}失败，源文件路径: {}, 源文件名称: {}, 目标文件路径: {}, 目标文件名称: {}, err: {}",
                dictMap.get(action),
                dictMap.get(type),
                srcPath,
                srcName,
          ...
          log.error(
              "批量{}{}失败，源文件路径: {}, 源文件名称: {}, 目标文件路径: {}, 目标文件名称: {}",
              dictMap.get(action),
              dictMap.get(type),
              srcPath,
              srcName,
  ...
  @CheckPassword(
      ...
      passwordFieldExpression = "[0].password")
  @DemoDisable
  public AjaxJson<Void> compressFile(@Valid @RequestBody CompressFileRequest request) {
    String compressTypeString = request.getCompressType();
    if (!CompressTypeEnum.isValid(compressTypeString)) {
      return AjaxJson.getError("不支持的压缩包类型");
    }
    CompressTypeEnum compressType = CompressTypeEnum.of(compressTypeString);

    AbstractBaseFileService<?> fileService =
        StorageSourceContext.getByStorageKey(request.getStorageKey());
    long splitSizeMB = request.getSplitSizeMB();
    log.info("分卷大小 %d", splitSizeMB);

    boolean success =
        fileService.compressFile(
            request.getPath(),
            request.getNameList(),
            request.getCompressName(),
            compressType,
            request.getSplitSizeMB());
    if (success) {
      return AjaxJson.getSuccess();
    }
    return AjaxJson.getError("创建压缩包失败");
  }

  @ApiOperationSupport(order = 8)
  @Operation(summary = "解压")
  ...
  @CheckPassword(
      storageKeyFieldExpression = "[0].storageKey",
      pathFieldExpression = "[0].output",
      passwordFieldExpression = "[0].outputPassword")
  @DemoDisable
  public AjaxJson<Void> decompressFile(@Valid @RequestBody DecompressFileRequest request) {
    String name = request.getName();
    log.info("decompressFile called. name: {}", name);
    String ext = FileUtil.extName(name);
    if (!CompressTypeEnum.isValid(ext)) {
      return AjaxJson.getError("不支持的压缩包类型");
    }
    CompressTypeEnum compressType = CompressTypeEnum.of(ext);

    String charsetCode = request.getCharset();
    if (!CharsetEnum.isValid(charsetCode)) {
      return AjaxJson.getError("不支持的字符集类型");
    }
    Charset charset = CharsetEnum.of(charsetCode).toCharset();

    AbstractBaseFileService<?> fileService =
        StorageSourceContext.getByStorageKey(request.getStorageKey());

    boolean success =
        fileService.decompressFile(
            request.getPath(),
            request.getName(),
            compressType,
            request.getOutput(),
            request.getCreateDir(),
            request.getPassword(),
            charset);
    if (success) {
      return AjaxJson.getSuccess();
    }
    return AjaxJson.getError("解压失败");
  }
}
