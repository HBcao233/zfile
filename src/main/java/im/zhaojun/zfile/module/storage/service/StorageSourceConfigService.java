...
public class StorageSourceConfigService {
  ...
  @Resource private StorageSourceConfigMapper storageSourceConfigMapper;
  ...
  public StorageSourceConfig findByStorageIdAndName(Integer storageId, String name) {
    return ((StorageSourceConfigService) AopContext.currentProxy())
        .selectStorageConfigByStorageId(storageId).stream()
  ...
  public void onStorageSourceDelete(StorageSourceDeleteEvent storageSourceDeleteEvent) {
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          "删除存储源 [id {}, name: {}, type: {}] 时，关联删除存储源参数 {} 条",
          storageId,
  ...
  public void saveBatch(Integer storageId, Collection<StorageSourceConfig> configList) {
    ...
    configList.forEach(
        storageSourceConfig -> {
          ...
          if (log.isDebugEnabled()) {
            log.debug(
                "新增存储源参数配置, 存储源 ID: {}, 存储源类型: {}, 参数名: {}",
                storageSourceConfig.getStorageId(),
                storageSourceConfig.getType().getDescription(),
   ...
   *
   * @param storageId 存储源 ID
   * @param storageType 存储源类型
   * @param storageSourceAllParam 存储源所有参数
   */
  public List<StorageSourceConfig> storageSourceAllParamToConfigList(
      Integer storageId,
      StorageTypeEnum storageType,
      StorageSourceAllParamDTO storageSourceAllParam) {
    // 返回结果
    List<StorageSourceConfig> result = new ArrayList<>();

    // 获取该存储源类型需要的参数列表
    List<StorageSourceParamDef> storageSourceParamList =
        StorageSourceContext.getStorageSourceParamListByType(storageType);

    // 遍历参数列表, 将参数转换成存储源参数对象
    for (StorageSourceParamDef storageSourceParam : storageSourceParamList) {
      // 根据字段名称获取字段值
      Object fieldValue =
          ReflectUtil.getFieldValue(storageSourceAllParam, storageSourceParam.getKey());
  ...
  public void onStorageSourceCopy(StorageSourceCopyEvent storageSourceCopyEvent) {
    ...
    List<StorageSourceConfig> storageSourceConfigList =
        ((StorageSourceConfigService) AopContext.currentProxy())
    ...
    log.info(
        "复制存储源 ID 为 {} 的存储源参数配置到存储源 ID 为 {} 成功, 共 {} 条",
        fromId,
        newId,
