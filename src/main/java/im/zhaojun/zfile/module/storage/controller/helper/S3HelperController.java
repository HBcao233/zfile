...
public class S3HelperController {
  ...
  public AjaxJson<List<S3BucketNameResult>> getBucketNames(
      @Valid @RequestBody GetS3BucketListRequest getS3BucketListRequest) {
    ...
    for (Bucket bucket : buckets) {
      S3BucketNameResult s3BucketNameResult =
          new S3BucketNameResult(bucket.name(), Date.from(bucket.creationDate()));
  ...
  public AjaxJson<List<ZFileCORSRule>> getCorsConfig(
      @Valid @RequestBody GetS3CorsListRequest getS3CorsListRequest) {
    ...
    try {
      ...
      GetBucketCorsResponse getBucketCorsResponse =
          s3Client.getBucketCors(builder -> builder.bucket(bucketName));
