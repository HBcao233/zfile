...
public class UserService {
  ...
  public List<UserDetailResponse> listUserDetail(QueryUserRequest queryObj) {
    ...
    LambdaQueryWrapper<User> queryWrapper =
        new QueryWrapper<User>()
            .orderBy(
                true, queryObj.getSortAsc(), StringUtils.camelToUnderline(queryObj.getSortField()))
            .lambda()
            .like(
                ObjUtil.isNotEmpty(queryObj.getUsername()),
                User::getUsername,
                queryObj.getUsername())
            .like(
                ObjUtil.isNotEmpty(queryObj.getNickname()),
                User::getNickname,
    ...
    if (queryObj.getHideDisabledStorage()) {
      userDetailResponseList.forEach(
          userDetailResponse -> {
            userDetailResponse
                .getUserStorageSourceList()
                .removeIf(userStorageSourceDetailDTO -> !userStorageSourceDetailDTO.getEnable());
          });
    }
   ...
   * 保存或更新用户
   *
   * @param saveUserRequest 用户信息
   * @return 保存后的用户
   */
  @CacheEvict(allEntries = true)
  public User saveOrUpdate(SaveUserRequest saveUserRequest) {
    // 校验用户是否存在
    boolean userNameIsDuplicate =
        checkDuplicateUsername(saveUserRequest.getId(), saveUserRequest.getUsername());
   ...
   *
   * @param updateUserPwdRequest 修改密码请求对象
   */
  @CacheEvict(allEntries = true)
  ...
  public void updateUserNameAndPwdById(Integer id, UpdateUserPwdRequest updateUserPwdRequest) {
    ...
    if (StringUtils.isNotBlank(user.getPassword())
        && !PasswordVerifyUtils.verify(
            user.getPassword(), user.getSalt(), updateUserPwdRequest.getOldPassword())) {
   ...
   * 检查用户名是否重复
   *
   * @param ignoreUserId 忽略的用户 ID, 用于检查重复时, 排除自身.
  ...
  public Integer copy(CopyUserRequest copyUserRequest) {
    ...
    log.info(
        "复制用户成功，源 [id: {}, username: {}, nickname: {}], 复制后 [id: {}, username: {}, nickname: {}]",
        fromUserId,
        user.getUsername(),
        user.getNickname(),
        newUserId,
