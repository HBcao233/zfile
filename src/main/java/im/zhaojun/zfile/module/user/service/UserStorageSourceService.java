...
public class UserStorageSourceService {
  ...
  @Resource private UserStorageSourceMapper userStorageSourceMapper;

  /**
   * 根据用户 ID 和存储策略 ID 查询存储策略权限
   *
   * @param userId 用户 ID
   * @param storageId 存储策略 ID
   * @return 存储策略权限
   */
  @Cacheable(
      key = "#userId + '-' + #storageId",
      unless = "#result == null",
   ...
   *
   * @param storageId 存储策略 ID
   * @param operatorTypeEnum 操作类型
   * @return 当前登录用户在指定存储策略是否有指定操作的权限
   */
  public boolean hasCurrentUserStorageOperatorPermission(
      Integer storageId, FileOperatorTypeEnum operatorTypeEnum) {
    UserStorageSource userStorageSource =
        ((UserStorageSourceService) AopContext.currentProxy())
            .getByUserIdAndStorageId(ZFileAuthUtil.getCurrentUserId(), storageId);
  ...
  public HashMap<String, Boolean> getCurrentUserPermissionMapByStorageId(Integer storageId) {
    ...
    for (FileOperatorTypeEnum operatorTypeEnum : FileOperatorTypeEnum.values()) {
      map.put(
          operatorTypeEnum.getValue(),
          userStoragePermissionsSet.contains(operatorTypeEnum.getValue()));
    }
  ...
  public void onStorageSourceDelete(StorageSourceDeleteEvent storageSourceDeleteEvent) {
    ...
    int updateRows =
        ((UserStorageSourceService) AopContext.currentProxy()).deleteByStorageId(storageId);
    log.info(
        "删除存储源 [id {}, name: {}, type: {}] 时，关联删除存储源用户权限 {} 条",
        storageId,
  ...
  public void onStorageSourceCopy(StorageSourceCopyEvent storageSourceCopyEvent) {
    ...
    List<UserStorageSource> userStorageSourceList =
        userStorageSourceMapper.selectByStorageId(fromId);

    userStorageSourceList.forEach(
        userStorageSource -> {
    ...
    log.info(
        "复制存储源 ID 为 {} 的存储源用户权限设置到存储源 ID 为 {} 成功, 共 {} 条",
        fromId,
        newId,
        userStorageSourceList.size());
  }

  /**
   * 监听用户复制事件, 复制用户时, 复制原用户的存储源权限到新用户
   *
   * @param userCopyEvent 用户复制事件
   */
  @EventListener
  public void onUserCopy(UserCopyEvent userCopyEvent) {
    ...
    log.info(
        "复制 ID 为 {} 的存储源用户权限设置到用户 ID 为 {} 成功, 共 {} 条", fromId, newId, userStorageSourceList.size());
  }
}
