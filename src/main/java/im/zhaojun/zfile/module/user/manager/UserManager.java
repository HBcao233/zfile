...
public class UserManager {
   ...
   *
   * @param user 用户
   * @param userStorageSourceList 用户存储策略权限列表
   */
  @Caching(
      evict = {
        @CacheEvict(
            cacheNames = UserStorageSourceService.USER_STORAGE_SOURCE_CACHE_KEY,
            allEntries = true)
      })
  ...
  public UserDetailResponse assembleUserDetail(User user) {
    Integer userId = user.getId();
    // 获取用户与存储策略的关联关系
    List<UserStorageSourceDetailDTO> userStorageListByUserId =
        userStorageSourceMapper.getDTOListByUserId(userId);
  ...
  public void addDefaultPermissionsForAllUsersInStorageSource(Integer storageId) {
    ...
    for (User user : users) {
      ...
      userStorageSourceMapper.insert(userStorageSource);
      log.info(
          "为用户添加存储源的默认权限: username: {}, storageId: {}, defaultPermissions: {}",
          user.getUsername(),
          storageId,
          defaultPermissions);
    }
