...
public class ReadmeConfigService {
  ...
  @Resource private ReadmeConfigMapper readmeConfigMapper;
   ...
   * @return 存储源文档配置列表
   */
  @Cacheable(key = "#storageId", condition = "#storageId != null")
   ...
   * @param readmeConfigList 存储源 readme 配置列表
   */
    ...
    readmeConfigList.forEach(
        readmeConfig -> {
          ...
          if (log.isDebugEnabled()) {
            log.debug(
                "新增目录文档, 存储源 ID: {}, 表达式: {}, 描述: {}, 显示模式: {}",
                readmeConfig.getStorageId(),
                readmeConfig.getExpression(),
                readmeConfig.getDescription(),
                readmeConfig.getDisplayMode().getValue());
          }
        });
  }
  ...
  public void onStorageSourceDelete(StorageSourceDeleteEvent storageSourceDeleteEvent) {
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          "删除存储源 [id {}, name: {}, type: {}] 时，关联删除存储源目录文档设置 {} 条",
          storageId,
  ...
  public ReadmeConfig findReadmeByPath(Integer storageId, String path) {
    List<ReadmeConfig> readmeConfigList =
        ((ReadmeConfigService) AopContext.currentProxy()).findByStorageId(storageId);
   ...
   */
  public ReadmeConfig getByStorageAndPath(
      Integer storageId, String path, Boolean compatibilityReadme) {
    ...
    if (BooleanUtils.isTrue(compatibilityReadme)) {
      ...
      // 获取指定目录 readme 文件
      ReadmeConfig dbReadmeConfig =
          ((ReadmeConfigService) AopContext.currentProxy()).findReadmeByPath(storageId, path);
   ...
   * @return 是否显示
   */
  private ReadmeConfig getReadmeByTestPattern(
      Integer storageId, List<ReadmeConfig> patternList, String test) {
    // 如果目录文档规则为空, 则可直接返回空.
    ...
    for (ReadmeConfig readmeConfig : patternList) {
      ...
      if (StringUtils.isEmpty(expression)) {
        if (log.isDebugEnabled()) {
          log.debug(
              "存储源 {} 目录文档规则表达式为空: {}, 测试字符串: {}, 表达式为空，跳过该规则比对", storageId, expression, test);
        }
        continue;
      }
      ...
      try {
        ...
        if (pathMode == ReadmePathModeEnum.ABSOLUTE) {
          ...
          match =
              PatternMatcherUtils.testCompatibilityGlobPattern(
                  expression, StringUtils.concat(currentUserBasePath, test));
        } else {
        ...
        if (log.isDebugEnabled()) {
          log.debug(
              "存储源 {} 目录文档规则表达式: {}, 测试字符串: {}, 匹配结果: {}", storageId, expression, test, match);
        }
  ...
  public void onStorageSourceCopy(StorageSourceCopyEvent storageSourceCopyEvent) {
    ...
    List<ReadmeConfig> readmeConfigList =
        ((ReadmeConfigService) AopContext.currentProxy()).findByStorageId(fromId);
    ...
    log.info(
        "复制存储源 ID 为 {} 的存储源目录文档设置到存储源 ID 为 {} 成功, 共 {} 条", fromId, newId, readmeConfigList.size());
  }
}
