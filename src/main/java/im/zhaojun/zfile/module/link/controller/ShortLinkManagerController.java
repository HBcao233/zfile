...
public class ShortLinkManagerController {
  ...
  @Resource private ShortLinkService shortLinkService;

  @Resource private StorageSourceService storageSourceService;

  @Resource private ShortLinkConvert shortLinkConvert;

  @Resource private LinkRateLimiterCache linkRateLimiterCache;
  ...
  public AjaxJson<List<ShortLinkResult>> list(QueryShortLinkLogRequest queryObj) {
    ...
    resultPage
        ...
        .forEach(
            shortLinkResult -> {
              shortLinkResult.setShortLink(
                  StringUtils.concat(serverAddress, "s", shortLinkResult.getShortKey()));
            });
  ...
  public void exportExcel(QueryShortLinkLogRequest queryObj, HttpServletResponse response)
      throws IOException {
  ...
  @NotNull private Page<ShortLinkResult> getShortLinkResultPage(QueryShortLinkLogRequest queryObj) {
    // 分页和排序
    boolean asc = Objects.equals(queryObj.getOrderDirection(), "asc");
    OrderItem orderItem =
        asc ? OrderItem.asc(queryObj.getOrderBy()) : OrderItem.desc(queryObj.getOrderBy());
    Page<ShortLink> pages =
        new Page<ShortLink>(queryObj.getPage(), queryObj.getLimit()).addOrder(orderItem);

    // 搜索条件
    LambdaQueryWrapper<ShortLink> queryWrapper =
        new LambdaQueryWrapper<ShortLink>()
            .eq(
                StringUtils.isNotEmpty(queryObj.getStorageId()),
                ShortLink::getStorageId,
                queryObj.getStorageId())
            .like(
                StringUtils.isNotEmpty(queryObj.getKey()),
                ShortLink::getShortKey,
                queryObj.getKey())
            .like(StringUtils.isNotEmpty(queryObj.getUrl()), ShortLink::getUrl, queryObj.getUrl())
            .ge(
                ObjUtil.isNotEmpty(queryObj.getDateFrom()),
                ShortLink::getCreateDate,
                queryObj.getDateFrom())
            .le(
                ObjUtil.isNotEmpty(queryObj.getDateTo()),
                ShortLink::getCreateDate,
                queryObj.getDateTo());
    ...
    Stream<ShortLinkResult> shortLinkResultList =
            ...
            .map(
                shortLink -> {
                  ...
                  StorageSource storageSource =
                      cache.getOrDefault(
                          shortLinkStorageId, storageSourceService.findById(shortLinkStorageId));
