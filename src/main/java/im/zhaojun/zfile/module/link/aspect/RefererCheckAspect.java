 ...
 * 校验 referer 防盗链.
 *
 * <p>校验所有标注了 {@link RefererCheck} 的注解
 *
 * @author zhaojun
 */
@Aspect
...
public class RefererCheckAspect {
  ...
  public Object around(ProceedingJoinPoint point) throws Throwable {
    ...
    if (!refererAllowEmpty && StringUtils.isEmpty(referer)) {
      ...
      return null;
    } else if (refererAllowEmpty
        && StringUtils.isEmpty(referer)) { // 如果 referer 允许为空，且当前 referer 为空，则跳过校验
      return point.proceed();
    }
    ...
    if (refererType == RefererTypeEnum.WHITE_LIST
        && containsPathMatcher(refererValueList, referer) == null) {
    ...
    if (refererType == RefererTypeEnum.BLACK_LIST
        && containsPathMatcher(refererValueList, referer) != null) {
