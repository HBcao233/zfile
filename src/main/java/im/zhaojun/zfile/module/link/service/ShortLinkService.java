...
public class ShortLinkService {
   ...
   * @return 短链接信息
   */
  public @Nullable ShortLink findByStorageIdAndUrl(
      Integer storageId, String url, @Nullable Date expireDate) {
  ...
  public ShortLink generatorShortLink(Integer storageId, String fullPath, Long expireTime) {
    ...
    if (Objects.equals(expireTime, ShortLink.PERMANENT_EXPIRE_TIME)) {
      ShortLink shortLink =
          findByStorageIdAndUrl(storageId, fullPath, ShortLink.PERMANENT_EXPIRE_DATE);
      if (shortLink != null) {
        return shortLink;
      }
    }
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          "生成直/短链: 存储源 ID: {}, 文件路径: {}, 短链 key {}, 随机生成直链冲突次数: {}",
  ...
  public void onStorageSourceDelete(StorageSourceDeleteEvent storageSourceDeleteEvent) {
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          "删除存储源 [id {}, name: {}, type: {}] 时，关联删除存储源短链 {} 条",
          storageId,
