...
public class LinkDownloadService {
  ...
  public ResponseEntity<?> handlerShortLink(String shortKey) throws IOException {
    ...
    return handlerDownloadGetUrl(
        storageKey, filePath, shortKey, DownloadLog.DOWNLOAD_TYPE_SHORT_LINK);
  }
   ...
   * @param filePath 文件路径
   * @param shortKey 短链接 key
   * @param downloadType 下载类型, 直链下载(directLink)或短链下载(shortLink)
   */
  private ResponseEntity<?> handlerDownloadGetUrl(
      String storageKey, String filePath, String shortKey, String downloadType) {
    String fileAlias =
        StringUtils.equals(downloadType, DownloadLog.DOWNLOAD_TYPE_DIRECT_LINK)
            ? filePath
            : shortKey;
    ...
    try {
      ...
      throw new ErrorPageBizException(
          "无效的或初始化失败的存储源 [" + storageKey + "] 文件 [" + fileAlias + "] 下载链接异常, 无法下载.", e);
    }

    if (fileService == null) {
      throw new ErrorPageBizException(
          "未找到存储源 [" + storageKey + "] 文件 [" + fileAlias + "] 下载链接异常, 无法下载.");
    }
    ...
    if (!enable) {
      throw new ErrorPageBizException(
          "未启用的存储源 [" + storageKey + "] 文件 [" + fileAlias + "] 下载链接异常, 无法下载.");
    }
    ...
    try {
      ...
      throw new ErrorPageBizException(
          "获取存储源 [" + storageKey + "] 文件 [" + fileAlias + "] 下载链接异常, 无法下载.", e);
    }

    // 判断下载链接是否为空
    if (StringUtils.isEmpty(downloadUrl)) {
      throw new ErrorPageBizException(
          "获取存储源 [" + storageKey + "] 文件 [" + fileAlias + "] 下载链接为空, 无法下载.");
    }
    ...
    if (StringUtils.equalsIgnoreCase(FileUtil.extName(filePath), "m3u8")) {
      ...
      return ResponseEntity.ok()
          ...
          .header(
              ...
              "attachment;filename="
                  + StringUtils.encodeAllIgnoreSlashes(FileUtils.getName(filePath)))
          .body(textContent);
    }
