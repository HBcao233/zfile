...
public class DirectLinkController {
  ...
  @Resource private LinkDownloadService linkDownloadService;

  @Resource private SystemConfigService systemConfigService;

  @Resource private DynamicDirectLinkPrefixService dynamicDirectLinkPrefixService;
  ...
  public void init() throws NoSuchMethodException {
    ...
    RequestMappingInfo requestMappingInfo =
        RequestMappingInfo.paths(directLinkPrefix + DIRECT_LINK_SUFFIX_PATH).build();
    dynamicDirectLinkPrefixService.registerMappingHandlerMapping(
        SystemConfig.DIRECT_LINK_PREFIX_NAME, requestMappingInfo, this, directLinkMethod);
  }
   ...
   */
  public ResponseEntity<?> directLink(@PathVariable("storageKey") String storageKey)
      throws IOException {
    // 获取直链全路径
  ...
  public AjaxJson<List<BatchGenerateLinkResponse>> generatorShortLink(
      @RequestBody @Valid BatchGenerateLinkRequest batchGenerateLinkRequest) {
    ...
    for (String path : batchGenerateLinkRequest.getPaths()) {
      ...
      String fullPath =
          StringUtils.concat(
              serverAddress, directLinkPrefix, storageKey, currentUserBasePath, path);
