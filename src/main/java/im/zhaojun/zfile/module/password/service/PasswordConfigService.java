...
public class PasswordConfigService {
  ...
  @Resource private PasswordConfigMapper passwordConfigMapper;
   ...
   *
   * @param storageId 存储源 ID
   * @param passwordConfigList 存储源类别
   */
    ...
    passwordConfigList.forEach(
        passwordConfig -> {
          ...
          if (log.isDebugEnabled()) {
            log.debug(
                "新增过滤规则, 存储源 ID: {}, 表达式: {}, 描述: {}, 密码: {}",
                passwordConfig.getStorageId(),
                passwordConfig.getExpression(),
                passwordConfig.getDescription(),
  ...
  public void onStorageSourceDelete(StorageSourceDeleteEvent storageSourceDeleteEvent) {
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          "删除存储源 [id {}, name: {}, type: {}] 时，关联删除存储源密码设置 {} 条",
          storageId,
  ...
  public VerifyResultDTO verifyPassword(Integer storageId, String path, String inputPassword) {
    ...
    List<PasswordConfig> passwordConfigList =
        ((PasswordConfigService) AopContext.currentProxy()).findByStorageId(storageId);
      ...
      try {
        ...
        if (match) {
          if (StringUtils.isEmpty(inputPassword)) {
            if (log.isDebugEnabled()) {
              log.debug(
                  "密码规则匹配, 但未输入密码；" + "表达式: {}, 请求路径: {}, 存储源 ID: {}, 预期密码：{}, 输入密码: {}",
                  expression,
                  path,
                  storageId,
                  expectPassword,
          ...
          if (matchPassword(expectPassword, inputPassword)) {
            if (log.isDebugEnabled()) {
              log.debug(
                  "密码规则匹配, 密码校验通过；" + "表达式: {}, 请求路径: {}, 存储源 ID: {}, 预期密码：{}, 输入密码: {}",
                  expression,
                  path,
                  storageId,
                  expectPassword,
          ...
          if (log.isDebugEnabled()) {
            log.debug(
                "密码规则匹配, 但输入密码与预期密码不同；" + "表达式: {}, 请求路径: {}, 存储源 ID: {}, 预期密码：{}, 输入密码: {}",
                expression,
                path,
                storageId,
                expectPassword,
        ...
        log.error(
            "密码规则匹配出现异常，表达式: {}, 请求路径: {}, 存储源 ID: {}, 预期密码：{}, 输入密码: {}, 解析错误, 跳过此规则.",
            expression,
            path,
            storageId,
            expectPassword,
  ...
  public void onStorageSourceCopy(StorageSourceCopyEvent storageSourceCopyEvent) {
    ...
    List<PasswordConfig> passwordConfigList =
        ((PasswordConfigService) AopContext.currentProxy()).findByStorageId(fromId);
    ...
    log.info(
        "复制存储源 ID 为 {} 的存储源密码设置到存储源 ID 为 {} 成功, 共 {} 条", fromId, newId, passwordConfigList.size());
  }
}
