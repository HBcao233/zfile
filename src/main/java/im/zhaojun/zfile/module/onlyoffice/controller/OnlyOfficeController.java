...
public class OnlyOfficeController {
  ...
  public AjaxJson<JSONObject> getPreviewFileJSONInfo(
      @Valid @RequestBody FileItemRequest fileItemRequest) {
    ...
    OnlyOfficeFile onlyOfficeFile =
        new OnlyOfficeFile(fileItemRequest.getStorageKey(), fileItemRequest.getPath());
  ...
  private Pair<FileItemResult, Boolean> getFileInfo(FileItemRequest fileItemRequest) {
    ...
    try {
      ...
      boolean hasUploadPermission =
          userStorageSourceService.hasCurrentUserStorageOperatorPermission(
              storageId, FileOperatorTypeEnum.UPLOAD);
  ...
  /**
   * 生成 OnlyOffice 预览所需的 JSON 数据. 配置参考: <a
   * href="https://api.onlyoffice.com/zh/editors/config/editor" />
   *
   * @param fileItemResult 文件信息
   * @param key OnlyOffice JWT 密钥
   * @param hasUploadPermission 是否有上传(编辑)权限
   * @return OnlyOffice 预览所需的 JSON 数据, 包含 JWT 密钥
   */
  private JSONObject createOnlyOfficePayload(
      FileItemResult fileItemResult, String key, boolean hasUploadPermission) {
    ...
    jsonObject.put(
        ...
        new JSONObject()
            ...
            .fluentPut("key", key)
            .fluentPut("permissions", new JSONObject().fluentPut("edit", hasUploadPermission))
    ...
    jsonObject.put(
        ...
        new JSONObject()
            .fluentPut(
                "callbackUrl",
                StringUtils.concat(
                    systemConfigService.getAxiosFromDomainOrSetting(), "/onlyOffice/callback"))
            .fluentPut("lang", "zh-CN")
            .fluentPut(
                "user",
                new JSONObject()
                    ...
                    .fluentPut(
                        "name",
                        StringUtils.firstNonNull(
                            currentUser.getNickname(), currentUser.getUsername()))));

    if (StringUtils.isNotEmpty(onlyOfficeSecret)) {
      String token =
          JWTUtil.createToken(jsonObject, onlyOfficeSecret.getBytes(StandardCharsets.UTF_8));
  ...
  public String callBack(@RequestBody OnlyOfficeCallback onlyOfficeCallback) {
    ...
    boolean useOnlyOfficeSecret =
        StrUtil.isNotBlank(systemConfigService.getSystemConfig().getOnlyOfficeSecret());
    if (useOnlyOfficeSecret) {
      ...
      if (!JWTUtil.verify(
          onlyOfficeCallback.getToken(),
          StrUtil.bytes(
              systemConfigService.getSystemConfig().getOnlyOfficeSecret(),
    ...
    if (SUPPORTED_STATUS.contains(onlyOfficeCallback.getStatus())) {
      ...
      try {
        ...
        AbstractBaseFileService<?> storageServiceByKey =
            StorageSourceContext.getByStorageKey(onlyOfficeFile.getStorageKey());
        ...
        log.debug(
            "开始保存 OnlyOffice 文件: {}, {}",
            onlyOfficeFile.getStorageKey(),
            onlyOfficeFile.getPathAndName());

        if (Beans.isInstanceOf(storageServiceByKey, AbstractProxyTransferService.class)) {
          // 进行上传.
          AbstractProxyTransferService<?> proxyUploadService =
