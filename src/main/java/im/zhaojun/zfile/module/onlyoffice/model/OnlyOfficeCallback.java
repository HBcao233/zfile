...
public class OnlyOfficeCallback {
  ...
  /**
   * 定义文档的状态。 可以有以下值： 1 - 正在编辑文档， 2 - 文档已准备好保存， 3 - 发生文档保存错误， 4 - 文档已关闭，没有任何更改， 6 -
   * 正在编辑文档，但保存了当前文档状态， 7 - 强制保存文档时发生错误。
   */
  private int status;
  ...
  /**
   * 定义有文档更改历史的对象。 仅当 status 值等于 2 或 3 时，对象才存在。 它包含对象 changes 和 serverVersion，它们必须作为对象的属性 changes 和
   * serverVersion 以参数形式发送给 refreshHistory 方法。
   */
  ...
  /**
   * 定义当用户对文档执行操作时接收到的对象。type 字段值可以具有以下值： 0 - 用户断开与文档共同编辑的连接， 1 - 新用户连接到文档共同编辑， 2 - 用户单击 强制保存按钮。
   * userid 字段值是用户标识符。
   */
