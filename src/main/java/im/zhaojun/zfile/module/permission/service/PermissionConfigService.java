...
public class PermissionConfigService {
  ...
  @Resource private PermissionConfigMapper permissionConfigMapper;
  ...
  public synchronized List<PermissionConfig> findByStorageId(Integer storageId) {
    return ((PermissionConfigService) AopContext.currentProxy())
        .findByStorageIdNotThreadSafe(storageId);
  }

  /**
   * 根据存储源 ID 查询权限配置 提示：受 sqlite 限制, 多线程调用此方法会出现 "[SQLITE_BUSY] The database file is locked
   * (database is locked)" 错误) 建议使用 {@link PermissionConfigService#findByStorageId(Integer)}
   * 俩保证所有数据库都是现场安全的。
   *
   * @param storageId 存储源ID
   * @return 存储源权限配置列表
  ...
  public List<PermissionConfig> findByStorageIdNotThreadSafe(Integer storageId) {
    ...
    // 按照权限枚举顺序排序结果
    dbResult.sort(
        Comparator.comparingInt(permissionConfig -> permissionConfig.getOperator().ordinal()));
    return dbResult;
  }
  ...
  public void onStorageSourceDelete(StorageSourceDeleteEvent storageSourceDeleteEvent) {
    ...
    int updateRows =
        ((PermissionConfigService) AopContext.currentProxy()).deleteByStorageId(storageId);
    if (log.isDebugEnabled()) {
      log.debug(
          "删除存储源 [id {}, name: {}, type: {}] 时，关联删除存储源权限设置(老) {} 条",
          storageId,
