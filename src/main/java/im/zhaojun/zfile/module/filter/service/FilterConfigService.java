...
public class FilterConfigService {
  ...
  @Resource private FilterConfigMapper filterConfigMapper;
   ...
   * @return 存储源过滤规则配置列表
   */
  @Cacheable(key = "'filter-base-' + #storageId", condition = "#storageId != null")
   ...
   * @return 禁止访问的过滤规则列表
   */
  @Cacheable(key = "'filter-inaccessible-' + #storageId", condition = "#storageId != null")
   ...
   * @return 禁止下载的过滤规则列表
   */
  @Cacheable(key = "'filter-disable-download-' + #storageId", condition = "#storageId != null")
   ...
   *
   * @param storageId 存储源 ID
   * @param filterConfigList 存储源过滤规则配置列表
   */
  ...
  public void batchSave(Integer storageId, List<FilterConfig> filterConfigList) {
    ...
    for (FilterConfig filterConfig : filterConfigList) {
      ...
      if (log.isDebugEnabled()) {
        log.debug(
            "新增过滤规则, 存储源 ID: {}, 表达式: {}, 描述: {}, 隐藏模式: {}",
            filterConfig.getStorageId(),
            filterConfig.getExpression(),
            filterConfig.getDescription(),
            filterConfig.getMode().getValue());
      }
    }
  }
   ...
   *
   * @param storageSourceDeleteEvent 存储源删除事件
   */
  @EventListener
  public void onStorageSourceDelete(StorageSourceDeleteEvent storageSourceDeleteEvent) {
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          "删除存储源 [id {}, name: {}, type: {}] 时，关联删除存储源过滤条件设置 {} 条",
          storageId,
  ...
  public boolean checkFileIsInaccessible(Integer storageId, String path) {
    List<FilterConfig> filterConfigList =
        ((FilterConfigService) AopContext.currentProxy()).findByStorageIdAndInaccessible(storageId);
  ...
  public boolean checkFileIsHidden(Integer storageId, String fileName) {
    List<FilterConfig> filterConfigList =
        ((FilterConfigService) AopContext.currentProxy()).findByStorageId(storageId);
  ...
  public boolean checkFileIsDisableDownload(Integer storageId, String fileName) {
    List<FilterConfig> filterConfigList =
        ((FilterConfigService) AopContext.currentProxy())
            .findByStorageIdAndDisableDownload(storageId);
    String filePath = FileUtils.getParentPath(fileName);
    if (StringUtils.isEmpty(filePath)) {
      return testPattern(storageId, filterConfigList, fileName);
    } else {
      return testPattern(storageId, filterConfigList, fileName)
          || testPattern(storageId, filterConfigList, filePath);
    }
  }
  ...
  private boolean testPattern(Integer storageId, List<FilterConfig> patternList, String test) {
    ...
    boolean isIgnoreHidden =
        userStorageSourceService.hasCurrentUserStorageOperatorPermission(
            storageId, FileOperatorTypeEnum.IGNORE_HIDDEN);
    if (isIgnoreHidden) {
      if (log.isDebugEnabled()) {
    ...
    for (FilterConfig filterConfig : patternList) {
      ...
      try {
        ...
        if (log.isDebugEnabled()) {
          log.debug(
              "存储源 {} 过滤文件测试表达式: {}, 测试字符串: {}, 匹配结果: {}", storageId, expression, test, match);
        }
   ...
   *
   * @param storageSourceCopyEvent 存储源复制事件
   */
  @EventListener
  public void onStorageSourceCopy(StorageSourceCopyEvent storageSourceCopyEvent) {
    ...
    List<FilterConfig> filterConfigList =
        ((FilterConfigService) AopContext.currentProxy()).findByStorageId(fromId);
    ...
    log.info(
        "复制存储源 ID 为 {} 的存储源过滤条件设置到存储源 ID 为 {} 成功, 共 {} 条", fromId, newId, filterConfigList.size());
  }
}
